...
const GlobalElements = ({ t }) => {
  ...
  const handleSelectElement = (element, isEditMode = false, fromTable = false) => {
    ...
    // Update de gebreken correct onder de juiste categorie
    const initialGebreken = element.gebreken
      ? element.gebreken
      : {
          [element.type]: extractGebreken(
            element.name,
            element.type,
            element.material,
            inspectionTasks
  ...
  const handleMaterialChange = (e) => {
    ...
    // Zorg ervoor dat gebreken correct wordt toegewezen onder de juiste categorie
  ...
  const handleSelectGebrek = (category, gebrekName) => {
    const updatedGebreken = { ...newElement.gebreken };
    if (!updatedGebreken[category]) {
      updatedGebreken[category] = [];
    }

    if (updatedGebreken[category].some((g) => g.name === gebrekName)) {
      updatedGebreken[category] = updatedGebreken[category].filter(
        (g) => g.name !== gebrekName
      );
    } else {
      updatedGebreken[category].push({ name: gebrekName, severity: "gering" }); // Standaard ernst
    }
  ...
  const handleAddCustomGebrek = () => {
    ...
    updatedGebreken[category].push({
      name: customGebrek.trim(),
      severity: customGebrekSeverity,
    });
  ...
  const renderStepContent = (step) => {
    switch (step) {
      ...
      case 3:
        // Gebreken & Inspecties
        // We tonen gebreken gebaseerd op de geselecteerde element, type en materiaal
              ...
              {Object.keys(currentGebreken).map((category, catIndex) => (
                <Box key={catIndex} mb={2}>
                  ...
                  <Grid container spacing={1}>
                    {currentGebreken[category].map((gebrek, index) => (
                      <Grid item xs={12} sm={6} md={4} key={index}>
                        <Box display="flex" alignItems="center">
                          <FormControlLabel
                            control={
                              <Checkbox
                                checked={true} // Omdat we alleen relevante gebreken tonen
                                onChange={() => handleSelectGebrek(category, gebrek.name)}
                              />
                            }
                            label={gebrek.name}
                          />
                          ...
                          >
                            <Select
                              ...
                              onChange={(e) =>
                                handleChangeSeverity(category, gebrek.name, e.target.value)
                              }
                            >
                        ...
                        <Box display="flex" alignItems="center">
                          <FormControlLabel
                            control={
                              <Checkbox
                                checked={true}
                                onChange={() =>
                                  handleSelectGebrek("customCategory", gebrek.name)
                                }
                              />
                            }
                            label={gebrek.name}
                          ...
                          >
                            <Select
                              value={gebrek.severity}
                              onChange={(e) =>
                                handleChangeSeverity("customCategory", gebrek.name, e.target.value)
                              }
                            >
                              <MenuItem value="ernstig">Ernstig</MenuItem>
                              <MenuItem value="serious">Serious</MenuItem>
  ...
  const handleChangeSeverity = (category, gebrekName, newSeverity) => {
    const updatedGebreken = newElement.gebreken[category].map((g) =>
      g.name === gebrekName ? { ...g, severity: newSeverity } : g
    );
    updateGebreken(category, updatedGebreken);
  };
