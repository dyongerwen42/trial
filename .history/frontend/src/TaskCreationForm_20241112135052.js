...
const TaskCreationForm = () => {
  ...
  const handleElementChange = useCallback(
    (event) => {
      ...
      // Ruimtenamen toewijzen aan elementen
  ...
  const handleAddTask = useCallback(() => {
    ...
    selectedElements.forEach((element) => {
      ...
      selectedTasks.forEach((task, taskIndex) => {
        ...
        let newEndDate = addDays(newStartDate, customTask.durationDays || 0);
        ...
        if (task.adjustments.setDirectToWorkDate && task.adjustments.workDate) {
          ...
          // Einddatum blijft ongewijzigd of kan aangepast worden via de eindDatum picker
        }
      ...
      case 0:
        ...
        return (
          <CardContent>
            ...
            {/* Datum Pickers voor Startdatum en Ultimata Datum */}
            <Box sx={{ mb: 3 }}>
            ...
            <Box sx={{ mb: 3 }}>
              ...
              <DatePicker
                ...
                onChange={(date) =>
                  setCustomTask((prev) => ({ ...prev, ultimateDate: date }))
                }
                dateFormat="dd/MM/yyyy"
                customInput={
                  <TextField
                    ...
                    variant="outlined"
                    size="medium"
                    placeholder="Selecteer ultimata datum"
                  />
                }
              />
            </Box>
              ...
              selectedTasks.map((task, index) => (
                <Accordion key={index} defaultExpanded>
                  ...
                  <AccordionDetails>
                    ...
                    />
                    {/* Werkdatum Picker - alleen zichtbaar als de checkbox is aangevinkt */}
                    ...
                    <TableBody>
                      {selectedElements.map((element) => {
                        ...
                        return selectedTasks.map((task, taskIndex) => {
                          ...
                          let newStartDate = new Date(startDate);
                          let newWorkDate = null;
                          let newEndDate = addDays(newStartDate, customTask.durationDays ? parseInt(customTask.durationDays) : 0);

                          if (offerAccepted) {
                            newUltimateDate = addDays(newStartDate, customTask.durationDays ? parseInt(customTask.durationDays) : 0);
                          }

                          // Werkdatum instellen indien geselecteerd
                          if (task.adjustments.setDirectToWorkDate && task.adjustments.workDate) {
                            newWorkDate = new Date(task.adjustments.workDate);
                            // Einddatum kan apart worden ingesteld
                          }
