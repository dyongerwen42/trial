...
import ExpandMoreIcon from '@mui/icons-materiala/ExpandMore';  // Icon for expanding
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} md={6}>
            <FormControl fullWidth variant="outlined" sx={{ mb: 2 }}>
              <InputLabel>Selecteer Taken</InputLabel>
              <Select
                label="Selecteer Taken"
                multiple
                value={newElement.selectedTasks || []}
                onChange={(e) => setNewElement({ ...newElement, selectedTasks: e.target.value })}
                renderValue={(selected) => selected.join(', ')}
              >
                {taken.map((task, index) => (
                  <MenuItem key={index} value={task.name}>
                    <Checkbox checked={newElement.selectedTasks?.indexOf(task.name) > -1} />
