...
const FullTaskManager = () => {
  ...
  const [{ isOver }, drop] = useDrop({
    accept: 'ELEMENT',
    drop: (item) => {
      // Voeg het element toe aan de geselecteerde categorie
      toggleElementSelection(item.category, item.id);
    },
    collect: (monitor) => ({
      ...
      {taskGroups.length > 0 ? (
        taskGroups.map((group) => (
          <Paper
            key={group.id}
            elevation={2}
            sx={{ mb: 2, p: 2, borderRadius: 2, position: 'relative' }}
          >
            ...
            <Grid container spacing={2}>
              {group.subtasks.map((element) => (
                <Grid item xs={12} md={6} key={element.id}>
                  <Paper
                    elevation={1}
                    sx={{ p: 2, borderRadius: 2, height: '100%', position: 'relative' }}
                  >
                    <Typography variant="subtitle1" sx={{ fontWeight: 500 }}>
                      {element.name}
                    </Typography>
