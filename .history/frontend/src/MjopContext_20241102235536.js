...
export const MjopProvider = ({ children }) => {
  ...
  const [mjopId, setMjopId] = useState(null); // Track the mjopId state
  ...
  useEffect(() => {
    ...
    setMjopId(id); // Update the state with the ID
  }, []);

  const initializeData = useCallback(async () => {
    if (!mjopId) {
      console.error('No MJOP ID available in query parameters');
      return;
    }

    try {
      console.log('Fetching MJOP data with ID:', mjopId); // Log ID
      const {
        mjop,
        generalInfo,
        cashInfo,
