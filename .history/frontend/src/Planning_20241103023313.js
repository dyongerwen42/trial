...
const Planning = () => {
  ...
  const {
    state: {
      globalSpaces = [], // Default to an empty array if undefined
      globalElements = [], // Default to an empty array if undefined
      cashInfo = { currentCash: 0, monthlyContribution: 0, reserveDate: new Date(), totalWorth: 0 }, // Default object with safe values
      offerGroups = [], // Default to an empty array if undefined
    },
    calculateCurrentCash,
    getSaldoColor,
    addTaskToPlanning,
    saveData,
    setOfferGroups,       // Added setter function
    setGlobalElements,    // Added setter function
  } = useMjopContext();
