...
export const MjopProvider = ({ children }) => {
  ...
  // Additional functions for InspectionReport
  const handleAddElement = (element) => {
    const elementWithId = { ...element, id: uuidv4() };
    logAction('Handle add element', { elementWithId });
    setGlobalElements([...state.globalElements, elementWithId]);
  };

  const handleEditElement = (element) => {
    logAction('Handle edit element', { element });
    setGlobalElements(
      state.globalElements.map((el) => (el.id === element.id ? element : el))
    );
  };

  const handleDeleteElement = (id) => {
    logAction('Handle delete element', { id });
    setGlobalElements(state.globalElements.filter((element) => element.id !== id));
  };
