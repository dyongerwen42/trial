...
const GlobalElements = ({ t }) => {
  ...
  const handleChangeSeverity = (category, oldSeverity, material, gebrekName, newSeverity) => {
    console.log("Handle Change Severity:", { category, severity: oldSeverity, material, gebrekName, newSeverity });
    if (oldSeverity === newSeverity) return; // Geen verandering

    // Verwijder het gebrek uit de huidige ernstcategorie
    removeGebrek(category, oldSeverity, material, gebrekName);
  ...
  const handleAddCustomGebrek = () => {
    ...
    const category = "customCategory"; // Je kunt een specifieke categorie voor aangepaste gebreken gebruiken
    const severityCategory = customGebrekSeverity;
    const material = "customMaterial"; // Of een andere logica voor materiaal
        ...
        {availableGebreken[severity].length > 0 ? (
          <Grid container spacing={1}>
            {availableGebreken[severity].map((gebrek) => {
              // Bepaal de huidige ernst van het gebrek
              const currentSeverity = Object.keys(newElement.gebreken[category]?.[material] || {}).find(sev =>
                newElement.gebreken[category][material][sev]?.includes(gebrek)
              ) || "gering";
                      ...
                      control={
                        <Checkbox
                          checked={
                            newElement.gebreken[category] &&
                            newElement.gebreken[category][material] &&
                            Object.keys(newElement.gebreken[category][material] || {}).some(sev =>
                              newElement.gebreken[category][material][sev]?.includes(gebrek)
                            )
                          }
                          onChange={() =>
                            handleSelectGebrek(category, currentSeverity, material, gebrek)
                          }
                        />
                      }
                      label={gebrek}
