...
const GlobalElements = ({ t }) => {
  ...
  // Save element with retry mechanism
  const handleSaveElement = async () => {
    ...
    try {
      if (isEditing) {
        await handleEditElement(newElement);
        console.log("Element succesvol bijgewerkt in context.");
        setSuccessMessage("Element succesvol bijgewerkt.");
      } else {
        await handleAddElement(newElement);
        console.log("Element succesvol toegevoegd aan context.");
        setSuccessMessage("Element succesvol toegevoegd.");
      }

      // Directly call saveData after adding/editing
      await attemptSaveDataWithRetry(3, 1000); // 3 retries, 1 second delay
  ...
  // Retry mechanism for saving data
  const attemptSaveDataWithRetry = async (retries, delay) => {
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        ...
        console.error(`Error saving element data to database (poging ${attempt}):`, error);
        if (attempt < retries) {
          console.log(`Proberen opnieuw in ${delay}ms...`);
          await new Promise((res) => setTimeout(res, delay));
        } else {
          setErrors({ general: "Error saving data to database after multiple attempts." });
          throw error;
        }
      }
    }
