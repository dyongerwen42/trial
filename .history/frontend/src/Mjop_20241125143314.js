...
const FullTaskManager = () => {
    ...
    const updatedGlobalElements = globalElements.map((element) => {
      if (selectedElementIds.includes(element.id)) {
        ...
        const newTask = {
          ...
          // urgency: Number(urgency), // Removed
          endDate,
  ...
  const filteredCategorizedElements = useMemo(() => {
    ...
    return Object.entries(categorizedElements).reduce(
      (acc, [category, spaces]) => {
        const filteredSpaces = Object.entries(spaces).reduce(
          (spaceAcc, [spaceName, elements]) => {
            const filteredElements = elements.filter((element) =>
              element.name.toLowerCase().includes(searchTerm.toLowerCase())
            );
            if (filteredElements.length > 0) {
              spaceAcc[spaceName] = filteredElements;
            }
            return spaceAcc;
          },
          {}
      ...
      <Grid container spacing={2} sx={{ height: '100%' }}>
        ...
        <Grid item xs={12} md={4}>
          ...
          >
            ...
            {Object.entries(filteredCategorizedElements).map(([category, spaces]) => {
              return (
                    ...
                    {Object.entries(spaces).map(([spaceName, elements]) => (
                      <Box key={spaceName} sx={{ mb: 2 }}>
                        ...
                        {elements.map((element) => {
                          // Calculate plannedYears per element
                          ...
                          return (
                            <Card
                              ...
                              sx={{
                                ...
                                backgroundColor: (selectedElementsByCategory[category] || []).includes(element.id)
                                  ? '#e3f2fd'
                                  : '#ffffff',
                            ...
                            >
                              <CardHeader
                                avatar={
                                  <Checkbox
                                    checked={(selectedElementsByCategory[category] || []).includes(element.id)}
                        ...
                        <Box key={group.id} sx={{ mb: 2 }}>
                          ...
                          >
                            <CardHeader
                              ...
                              // Removed: Edit IconButton from timeline cards
                            />
                            {/* Removed: Subtasks within timeline card */}
                          </Card>
                        </Box>
                      ))
                    ) : (
