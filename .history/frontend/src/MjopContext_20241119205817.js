...
const EDIT_TASK = "EDIT_TASK"; // Nieuw
const DELETE_TASK = "DELETE_TASK"; // Nieuw
...
const mjopReducer = (state, action) => {
  ...
  switch (action.type) {
      ...
      return {
        ...
        globalElements: Array.isArray(action.payload.globalElements)
          ? action.payload.globalElements.map((element) => ({
              ...
              tasks: Array.isArray(element.tasks) ? element.tasks : [], // Zorg ervoor dat tasks altijd een array is
      ...
      return {
        ...
        globalElements: [
          ...state.globalElements,
          {
            ...
            tasks: Array.isArray(action.payload.tasks) ? action.payload.tasks : [], // Voeg tasks toe
          ...
          el.id === action.payload.id
            ? {
                ...
                inspectionReport: Array.isArray(action.payload.inspectionReport)
                  ...
                  : el.inspectionReport,
                tasks: Array.isArray(action.payload.tasks) ? action.payload.tasks : el.tasks, // Bewaar bestaande tasks als niet geleverd
    ...
    case ADD_TASK:
      ...
      const updatedElementsWithTask = state.globalElements.map((element) =>
        element.id === action.payload.elementId
          ? {
              ...element,
              tasks: Array.isArray(element.tasks) ? [...element.tasks, ...action.payload.tasks] : [...action.payload.tasks],
            }
          : element
      );
      return {
    ...
    case EDIT_TASK:
      console.log("Reducer EDIT_TASK aangeroepen met:", action.payload);
      const updatedElementsWithEditedTask = state.globalElements.map((element) => {
      ...
      return {
        ...state,
        globalElements: updatedElementsWithEditedTask,
      };

    case DELETE_TASK:
      console.log("Reducer DELETE_TASK aangeroepen met:", action.payload);
      const updatedElementsWithDeletedTask = state.globalElements.map((element) => {
        if (element.id === action.payload.elementId) {
          return {
            ...element,
            tasks: Array.isArray(element.tasks)
      ...
      return {
        ...state,
        globalElements: updatedElementsWithDeletedTask,
      };
    ...
    case ADD_GEBREKEN:
      ...
      const updatedInspectionReportAddMultiple = state.newElement.inspectionReport.map((report) => ({
        ...report,
        mistakes: Array.isArray(report.mistakes) ? [...report.mistakes, ...newMistakes] : [...newMistakes],
...
export const MjopProvider = ({ children }) => {
  ...
  const saveData = useCallback(
    async (updatedGlobalElements = state.globalElements, skipValidation = false) => {
      try {
        ...
        // Verzeker je ervan dat 'gebreken' en 'tasks' correct zijn in updatedGlobalElements
        ...
        // Extra logging om te controleren of 'gebreken' en 'tasks' correct zijn
  ...
  // Functie om een item toe te voegen aan planning (taak)
  ...
  // Functie om een item te wijzigen (taak)
  const handleItemChange = useCallback(
    (taskId, field, value) => {
      logAction("Changing Item", { taskId, field, value });

      // Vind het element dat de taak bevat
      const element = state.globalElements.find((el) => el.tasks.some((task) => task.id === taskId));
      if (!element) {
        console.error(`Element niet gevonden voor taak ID: ${taskId}`);
        return;
      }

      dispatch({
        type: EDIT_TASK,
        payload: { elementId: element.id, task: { id: taskId, [field]: value } },
      });
    },
    [dispatch, state.globalElements, logAction]
  );

  // Functie om een item te verwijderen (taak)
  ...
  // Functie om een aangepast item toe te voegen (taak)
  const handleAddCustomItem = useCallback(
    (item) => {
      ...
      if (generalElement) {
        ...
        handleAddItemToPlanning(newTask, generalElement.id);
      } else {
    ...
    [dispatch, state.globalElements, logAction, handleAddItemToPlanning, handleAddElement]
  );
