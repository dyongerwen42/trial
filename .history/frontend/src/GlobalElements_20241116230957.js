...
const GlobalElements = ({ t }) => {
  ...
  // Initialize elements with unique IDs
  const initializedGlobalElements = useMemo(() => {
  ...
  const filteredElements = useMemo(() => {
    return initializedGlobalElements.filter(
      (element) =>
        element.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        element.description.toLowerCase().includes(searchTerm.toLowerCase())
    );
  }, [searchTerm, initializedGlobalElements]);
  ...
  const data = useMemo(() => initializedGlobalElements, [initializedGlobalElements]);
