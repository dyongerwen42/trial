...
const GlobalElements = ({ t }) => {
  ...
  const handleOpenDocumentsModal = useCallback((documents) => {
    console.log("Opening Documents Modal with documents:", documents);
  ...
  const handleOpenPhotosModal = useCallback((photos) => {
    console.log("Opening Photos Modal with photos:", photos);
    setSelectedPhotos(
      photos.map((photo) =>
        typeof photo === "string"
  ...
  const handleClosePhotosModal = useCallback(() => {
    console.log("Closing Photos Modal");
  ...
  const handleSelectElement = useCallback(
    (element, isEditMode = false, fromTable = false) => {
      console.log("Selecting element for editing:", element, "isEditMode:", isEditMode, "fromTable:", fromTable);
  ...
  const handleMaterialChange = useCallback(
    (e) => {
      ...
      console.log("Material changed to:", selectedMaterial);
  ...
  const handleFileChange = useCallback(
    async (e) => {
      console.log("Handling file change for photos");
  ...
  const handleDocumentChange = useCallback(
    async (e) => {
      console.log("Handling file change for documents");
      const files = Array.from(e.target.files);
      const fileUrls = await Promise.all(files.map((file) => uploadFile(file)));
      setNewElement((prev) => ({
        ...prev,
  ...
  const handleResetNewElement = useCallback(() => {
    console.log("Resetting newElement");
    resetNewElement();
    setImagePreviews([]);
    setDocumentPreviews([]);
  ...
  const handleNext = useCallback(() => {
    console.log("Navigating to next step from step:", activeStep);
  ...
  const handleTypeChangeFromSidePanel = useCallback(
    (element, selectedType) => {
      console.log("Handling type change from sidebar:", selectedType);
      const materials = Object.keys(
  ...
  const handleExpandClick = useCallback((index) => {
    console.log("Toggling expansion for element at index:", index);
  ...
  const handleSelectGebrek = useCallback(
    (category, severity, material, gebrekName) => {
      ...
      if (isSelected) {
        console.log("Removing gebrek:", gebrekName);
        removeGebrek(category, severity, material, gebrekName);
      } else {
        console.log("Adding gebrek:", gebrekName);
  ...
  const handleChangeSeverity = useCallback(
    (category, severity, material, gebrekName, newSeverity) => {
      ...
      console.log("Changing severity: Removing from", severity, "Adding to", newSeverity);
      removeGebrek(category, severity, material, gebrekName);
  ...
  const handleAddCustomGebrek = useCallback(() => {
    ...
    console.log("Adding custom gebrek:", customGebrek, "with severity:", customGebrekSeverity);
  ...
  const dataTable = globalElements;
  ...
  const renderGebreken = useCallback(() => {
    console.log("Rendering gebreken");
  ...
  const renderStepContent = useCallback((step) => {
    ...
    switch (step) {
      ...
      }, [newElement, errors, handleTypeChange, handleMaterialChange, extractGebreken, renderGebreken, handleSelectGebrek, handleChangeSeverity]);
