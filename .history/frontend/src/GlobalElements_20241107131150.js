...
const handleSelectElement = (element, isEditMode = false, fromTable = false) => {
  ...
  const handleSaveElement = async () => {
    // Define validation rules, checking only essential fields as needed
    const newErrors = {};
  
    // Validate only the name field, skip other fields unless required
    if (!newElement.name) newErrors.name = 'Naam is verplicht';
  
    // Set form feedback if any errors are found
    if (Object.keys(newErrors).length > 0) {
      setErrors(newErrors);
      setFoutieveStappen(newErrors.name ? [0] : []); // Highlight step 0 if `name` is missing
      return;
    }
  
    // Add or edit the element, saveData is handled within handleAddElement or handleEditElement
    if (isEditing) {
      handleEditElement(newElement); // Update the element
    } else {
      await handleAddElement(newElement); // Add a new element and save it to the backend
    }
