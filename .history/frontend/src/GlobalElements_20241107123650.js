...
const GlobalElements = ({ t }) => {
  const {
    state: { globalElements, globalSpaces, availableElements },
    handleAddElement,
    handleEditElement,
    handleDeleteElement,
  } = useMjopContext();
...
const handleSelectElement = (element, isEditMode = false, fromTable = false) => {
  ...
  const handleSaveElement = () => {
    ...
    if (!newElement.description) newErrors.description = 'Beschrijving is verplicht';
    if (!newElement.interval) newErrors.interval = 'Interval is verplicht';
    if (!newElement.spaceId) newErrors.spaceId = 'Ruimte is verplicht';
    if (!newElement.type) newErrors.type = 'Type is verplicht';
    if (!newElement.material) newErrors.material = 'Materiaal is verplicht';
    if (newElement.material === 'Other' && !newElement.customMaterial) newErrors.customMaterial = 'Aangepast materiaal is verplicht';

    if (Object.keys(newErrors).length > 0) {
      setErrors(newErrors);
      const invalidSteps = [];
      if (newErrors.name || newErrors.description) invalidSteps.push(0);
      if (newErrors.spaceId || newErrors.type || newErrors.material) invalidSteps.push(1);
