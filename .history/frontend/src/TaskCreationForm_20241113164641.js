  ...
  const handleAddTask = useCallback(() => {
    ...
    selectedElements.forEach((element) => {
      ...
      tasksToAdd.forEach((task, taskIndex) => {
        ...
        let taskDescription = task.adjustments?.description || task.description || task.toelichting;
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} md={6}>
                    <TextField
                      fullWidth
                      label="Beschrijving"
                      value={task.adjustments.description || task.toelichting || task.description}
                  ...
                  <DatePicker
                    selected={
                      ...
                      addDays(
                        ...
                        task.adjustments.durationDays ? parseInt(task.adjustments.durationDays) : 0
                      )
                    }
                    onChange={(date) =>
                ...
                <TableBody>
                  {selectedElements.map((element) => {
                    ...
                    return selectedTasks.map((task, taskIndex) => {
                      ...
                      let newUltimateDate = new Date(task.adjustments?.ultimateDate || new Date());
                      let newStartDate = task.adjustments?.startDate || new Date();
                      let newWorkDate = null;
                      let durationDays = task.adjustments?.durationDays
                        ? parseInt(task.adjustments.durationDays)
                      ...
                      return (
                        <TableRow key={`${element.id}-${taskIndex}`}>
                          <TableCell>{task.adjustments.name || task.beschrijving || task.name}</TableCell>
