...
export const MjopProvider = ({ children }) => {
  ...
  const setGlobalElements = useCallback((globalElements) => {
    console.log(globalElements,'fffffffffff')
  ...
  const initializeData = useCallback(async (id) => {
    try {
      ...
      console.log("[initializeData] Processed payload:", payload);
      console.log(`[initializeData] globalElements type: ${typeof payload.globalElements}, isArray: ${Array.isArray(payload.globalElements)}`);
      console.log(`[initializeData] globalSpaces type: ${typeof payload.globalSpaces}, isArray: ${Array.isArray(payload.globalSpaces)}`);
      console.log(`[initializeData] globalDocuments type: ${typeof payload.globalDocuments}, isArray: ${Array.isArray(payload.globalDocuments)}`);
      console.log(`[initializeData] offerGroups type: ${typeof payload.offerGroups}, isArray: ${Array.isArray(payload.offerGroups)}`);
  ...
  const handleAddTask = useCallback((taskData, useInterval) => {
    setGlobalElements((prevElements) =>
      prevElements.map((element) => {
        if (element.id !== taskData.elementId) return element;
        ...
        const createNewTask = (startDate, estimatedPrice) => ({
          id: uuidv4(),
          name: taskData.name,
          description: taskData.description,
          urgency: taskData.urgency,
          planned: {
            workDate: startDate.toISOString(),
          ...
          ultimateDate: new Date(
            startDate.getFullYear() + taskData.intervalYears,
            startDate.getMonth(),
            startDate.getDate()
          ).toISOString(),
        });
        ...
        if (useInterval) {
          for (let i = 0; i < taskData.totalYears; i += taskData.intervalYears) {
            updatedTasks.push(createNewTask(currentStartDate, currentEstimatedPrice));
