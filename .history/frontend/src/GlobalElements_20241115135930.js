...
const extractGebreken = (elementName, type, material, data, customGebreken) => {
  ...
  // Voeg aangepaste gebreken toe aan de beschikbare gebreken
  for (const severity in customGebreken) {
    if (!gebreken[severity]) {
      gebreken[severity] = [];
    }
    gebreken[severity] = gebreken[severity].concat(customGebreken[severity]);
  }
...
const GlobalElements = ({ t }) => {
  const {
    ...
    addGebrek,
    removeGebrek,
    addCustomGebrek, // Toegang tot addCustomGebrek
  } = useMjopContext();

  // Definieer de state variabelen voor aangepaste gebreken
  const [customGebrek, setCustomGebrek] = useState("");
  const [customGebrekSeverity, setCustomGebrekSeverity] = useState("gering"); // Consistente naam
  ...
  const handleSelectElement = (element, isEditMode = false, fromTable = false) => {
    ...
    // Haal alle beschikbare gebreken op
    ...
    // Stel initialGebreken in met alle beschikbare gebreken
    const initialGebreken = {};
    Object.keys(availableGebreken).forEach((severity) => {
      initialGebreken[severity] = availableGebreken[severity].slice(); // Clone array
    });
    ...
    setNewElement({
      ...
      gebreken: initialGebreken, // Alle gebreken zijn geselecteerd
  ...
  const handleAddCustomGebrek = () => {
    ...
    // Voeg het aangepaste gebrek toe aan de beschikbare gebrekenlijst
    addCustomGebrek(severity, gebrekName);
