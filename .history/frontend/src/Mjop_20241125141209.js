...
const FullTaskManager = () => {
  ...
  // State for Category Elements Dialog
  const [categoryDialogOpen, setCategoryDialogOpen] = useState(false);
  const [currentCategoryElements, setCurrentCategoryElements] = useState([]);
  ...
  // Handle Category Elements Dialog Open
  const handleOpenCategoryDialog = (elements) => {
    setCurrentCategoryElements(elements);
    setCategoryDialogOpen(true);
  };

  // Handle Category Elements Dialog Close
  const handleCloseCategoryDialog = () => {
    setCategoryDialogOpen(false);
    setCurrentCategoryElements([]);
  };
  ...
  return (
    <Box sx={{ position: 'relative', height: '100vh', backgroundColor: '#e0e0e0', p: 2 }}>
      <Grid container spacing={2} sx={{ height: '100%' }}>
        ...
        <Grid item xs={12} md={4}>
          ...
          >
            ...
            {/* Displaying Categories with Info Icons */}
            <List>
              {Object.keys(filteredCategorizedElements).map((category) => (
                <ListItem key={category} disableGutters>
                  ...
                  />
                  <ListItemSecondaryAction>
                    <Tooltip title="Bekijk elementen" arrow>
                      <IconButton
                        edge="end"
                        aria-label={`view elements in ${category}`}
                        onClick={() => handleOpenCategoryDialog(filteredCategorizedElements[category])}
                      >
                        <InfoOutlinedIcon />
                      </IconButton>
                    </Tooltip>
      ...
      <Dialog
        ...
        fullWidth
      >
        <DialogTitle>Elementen in {currentCategory}</DialogTitle>
        <DialogContent>
          {currentCategoryElements.length > 0 ? (
            <Grid container spacing={2}>
              {currentCategoryElements.map((element) => {
            ...
            <Typography variant="body2" color="textSecondary">
              Geen elementen beschikbaar in deze categorie.
            </Typography>
          )}
        </DialogContent>
        <DialogActions>
