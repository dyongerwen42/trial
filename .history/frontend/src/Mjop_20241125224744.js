...
const FullTaskManager = () => {
  ...
  const deleteTaskGroup = (groupId) => {
    ...
    setSnackbarMessage('Taakgroep succesvol verwijderd!');
    setSnackbarOpen(true);
  };
  ...
  // Categorize content into tabs
  const renderTaskGroupInfoTabs = () => {
    if (!currentTaskGroupInfo) return null;
    ...
    return (
      <Box sx={{ width: '100%' }}>
        <Tabs
          ...
          variant="fullWidth"
        >
          <Tab label="Overzicht" id="tab-0" aria-controls="tabpanel-0" />
          <Tab label="Subtaken" id="tab-1" aria-controls="tabpanel-1" />
          <Tab label="Instellingen" id="tab-2" aria-controls="tabpanel-2" />
        </Tabs>
        <Box role="tabpanel" hidden={selectedTab !== 0} id="tabpanel-0" aria-labelledby="tab-0" sx={{ p: 2 }}>
          {selectedTab === 0 && (
            <Box>
        ...
        <Box role="tabpanel" hidden={selectedTab !== 1} id="tabpanel-1" aria-labelledby="tab-1" sx={{ p: 2 }}>
          {selectedTab === 1 && (
            <Box>
        ...
        <Box role="tabpanel" hidden={selectedTab !== 2} id="tabpanel-2" aria-labelledby="tab-2" sx={{ p: 2 }}>
          {selectedTab === 2 && (
            <Box>
              ...
              <FormControlLabel
                control={
                  <Switch
                    checked={currentTaskGroupInfo.assignPricesIndividually}
                    onChange={(e) => {
                      const updatedTaskGroups = taskGroups.map((group) => {
                        if (group.id === currentTaskGroupInfo.id) {
                          return { ...group, assignPricesIndividually: e.target.checked };
                        }
                        return group;
                      });
                      setTaskGroups(updatedTaskGroups);
                      setCurrentTaskGroupInfo((prev) => ({
                        ...prev,
  ...
  return (
    <Box sx={{ position: 'relative', height: '100vh', backgroundColor: '#f4f6f8', p: 3 }}>
      ...
      >
        ...
        <DialogActions>
          ...
          <Button
            ...
            sx={{ textTransform: 'none' }}
            startIcon={<AddIcon fontSize="small" />}
            aria-label={isEditDialogOpen ? 'Taakgroep bijwerken' : 'Taakgroep toevoegen'}
          >
      ...
      {/* Task Group Info Dialog with Tabs */}
      <Dialog
        open={taskGroupInfoDialogOpen}
        onClose={handleCloseTaskGroupInfoDialog}
        maxWidth="md"
        fullWidth
        aria-labelledby="task-group-info-dialog-title"
      >
        <DialogTitle id="task-group-info-dialog-title">
      ...
      >
        ...
        <DialogContent>
          <Typography id="delete-confirmation-dialog-description">
            Weet je zeker dat je de taakgroep "{currentTaskGroupInfo?.name}" wilt verwijderen? Deze actie kan niet ongedaan worden gemaakt.
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button
          ...
          <Button
            onClick={() => deleteTaskGroup(currentTaskGroupInfo.id)}
            color="error"
            variant="contained"
            sx={{ textTransform: 'none' }}
