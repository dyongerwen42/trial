...
const GlobalSpaces = () => {
 ...
 const handleSelectSpace = (space) => {
  ...
  const handleSaveSpace = async () => {
    ...
    // Validate that name and description fields are filled
    if (!newSpace.name) newErrors.name = 'Naam is verplicht';
    if (!newSpace.description) newErrors.description = 'Beschrijving is verplicht';
  
    // Set any validation errors and mark steps as incomplete if errors exist
    setErrors(newErrors);
    handleFoutieveStappen(newErrors);
  
    if (Object.keys(newErrors).length > 0) return; // Exit if there are errors
  
    // Check if editing an existing space or adding a new space
    if (isEditing) {
      handleEditSpace(newSpace); // Update existing space in the context
    } else {
      handleAddSpace(newSpace); // Add new space to the context
    }
  
    // Clear errors and reset space data in the form
    setErrors({});
    setFoutieveStappen([]);
    resetNewSpace();
  
    // Call saveData from the context to persist data to the database
    try {
      await saveData();
      console.log("Space data saved to the database successfully.");
    } catch (error) {
      console.error("Error saving space data:", error);
      setErrors({ save: 'Error saving space data. Please try again.' });
    }
  };
  ...
  return (
    <Box display="flex" height="100vh">
      {isSidebarOpen && (
        <Paper
          sx={{
            flexBasis: '20%', // Use a smaller portion of the width
            p: 2,
            boxShadow: 4,
            borderRadius: 2,
            ...
            {filteredSpaces.map((space) => (
              <React.Fragment key={space.id}>
                <ListItem button onClick={() => handleSelectSpace(space)} sx={{ mb: 1 }}>
                  <ListItemText primary={space.name} /> {/* Removed the description */}
                </ListItem>
                <Divider />
              </React.Fragment>
            ))}
      ...
      <Box
        component="section"
        sx={{
          width: isSidebarOpen ? '80%' : '100%', // Main content takes remaining space
          minWidth: '0', // Prevent overflow
          display: 'flex',
          flexDirection: 'column',
          p: 3,
          transition: 'width 0.3s ease',
          flexGrow: 1, // Ensure it grows to take up the remaining space
        }}
      >
