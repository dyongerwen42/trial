                    ...
                    >
                      ...
                      <Grid item xs={12} sm={6}>
                        ...
                        >
                          ...
                          {/* Voeg een "Overig" optie toe */}
                          <MenuItem value="__OTHER__">Overig</MenuItem>
                        </TextField>
                        {/* Conditionally render the input field if "Overig" is selected */}
                        {mistake.category === "__OTHER__" && (
                          <TextField
                            fullWidth
                            label="Nieuwe Categorie"
                            value={mistake.newCategory || ""}
                            onChange={(e) => {
                              const newValue = e.target.value;
                              handleMistakeChange(index, "newCategory", newValue);
                            }}
                            variant="outlined"
                            sx={{ mt: 2 }}
                            aria-label={`new defect category ${index + 1}`}
                            onBlur={() => {
                              if (mistake.newCategory && mistake.newCategory.trim() !== "") {
                                // Update the category with the new value
                                handleMistakeChange(index, "category", mistake.newCategory.trim());
                                // Voeg de nieuwe categorie toe aan de globale context
                                addGebrekToElement(currentElementId, mistake.severity, [mistake.newCategory.trim()]);
                                // Optionally, clear the newCategory field
                                handleMistakeChange(index, "newCategory", "");
                              }
                            }}
                          />
                        )}
