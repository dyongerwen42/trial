...
export const MjopProvider = ({ children }) => {
  ...
  const handleAddSpace = useCallback((space) => {
    ...
    dispatch({ type: 'ADD_GLOBAL_SPACE', payload: spaceWithId }); // Update globalSpaces
  }, [dispatch, logAction]);

  const handleEditSpace = useCallback((space) => {
    logAction('Handle edit space', { space });
    dispatch({ type: 'EDIT_GLOBAL_SPACE', payload: space }); // Edit specific space
  }, [dispatch, logAction]);

  const handleDeleteSpace = useCallback((id) => {
    logAction('Handle delete space', { id });
    dispatch({ type: 'DELETE_GLOBAL_SPACE', payload: id }); // Remove space by ID
  }, [dispatch, logAction]);
