...
export const MjopProvider = ({ children }) => {
  ...
  const handleAddSpace = useCallback((space) => {
    const spaceWithId = { ...space, id: uuidv4() }; // Assign a unique ID
    logAction('Handle add space', { spaceWithId });
    dispatch({ type: 'SET_DATA', payload: { globalSpaces: [...state.globalSpaces, spaceWithId] } }); // Update globalSpaces
  }, [dispatch, state.globalSpaces, logAction]);

  const handleEditSpace = useCallback((space) => {
    logAction('Handle edit space', { space });
    dispatch({ type: 'SET_DATA', payload: { globalSpaces: state.globalSpaces.map(sp => (sp.id === space.id ? space : sp)) } }); // Edit specific space
  }, [dispatch, state.globalSpaces, logAction]);

  const handleDeleteSpace = useCallback((id) => {
    logAction('Handle delete space', { id });
    dispatch({ type: 'SET_DATA', payload: { globalSpaces: state.globalSpaces.filter(space => space.id !== id) } }); // Remove space by ID
  }, [dispatch, state.globalSpaces, logAction]);
