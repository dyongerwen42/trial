...
const FullTaskManager = () => {
  ...
  const addOrUpdateTaskGroup = useCallback(() => {
    ...
    if (dialogMode === 'add') {
      if (periodic) {
        ...
        let updatedGlobalElements = [...state.globalElements];
        ...
        for (let i = 0; i < totalPeriods; i++) {
          ...
          selectedElementIds.forEach((elementId) => {
            ...
            // Update globalElements met de nieuwe taak
            updatedGlobalElements = updatedGlobalElements.map((el) => {
              if (el.id === elementId) {
                return {
                  ...el,
        ...
        // Update de globale elementen buiten de loop
  ...
  }, [ // Correct closure of useCallback
    dialogMode,
    taskProperties,
