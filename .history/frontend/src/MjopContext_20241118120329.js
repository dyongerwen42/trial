...
const mjopReducer = (state, action) => {
  switch (action.type) {
    ...
    case "ADD_GEBREK_TO_ELEMENT":
      const { elementId, category: addCategory, material: addMaterial, severityCategory: addSeverityCategory, gebrekName: addGebrekName } = action.payload;

      return {
        ...state,
        globalElements: state.globalElements.map((element) => {
          if (element.id === elementId) {
            const existingGebreken = element.gebreken?.[addCategory]?.[addMaterial]?.[addSeverityCategory] || [];
            if (existingGebreken.includes(addGebrekName)) {
              console.warn(`Gebrek "${addGebrekName}" is al toegevoegd onder ${addSeverityCategory}.`);
              return element;
            }
            return {
              ...element,
              gebreken: {
                ...element.gebreken,
                [addCategory]: {
                  ...element.gebreken?.[addCategory],
                  [addMaterial]: {
                    ...element.gebreken?.[addCategory]?.[addMaterial],
                    [addSeverityCategory]: [
                      ...existingGebreken,
                      addGebrekName,
                    ],
