    ...
    const updatedGlobalElements = globalElements.map((element) => {
      if (selectedElementIds.includes(element.id)) {
        const newTask = {
          ...
          urgency: Number(urgency), // Ensure urgency is a number
          year,
          cost,
          isGrouped: false,
        };
        ...
        <DialogContent>
          {currentElementInfo ? (
            <Box>
                ...
                <Box sx={{ mt: 2 }}>
                  ...
                  <Box display="flex" flexWrap="wrap" gap={2} mt={1}>
                    {currentElementInfo.photos.map((photo, index) => (
                      <img
                        key={index}
                        src={photo.replace('\\\\', '/')} // Adjust path separators if necessary
                      ...
                      {task.images && task.images.length > 0 && (
                        <Box display="flex" flexWrap="wrap" gap={1} mt={1}>
                          {task.images.map((img, idx) => (
                            <img
                              key={idx}
                              src={img.replace('\\\\', '/')} // Adjust path separators if necessary
                              alt={`${task.name} ${idx + 1}`}
              ...
              {/* You can expand this section to include more data fields as required */}
            </Box>
          ) : (
