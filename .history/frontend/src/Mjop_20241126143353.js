...
const FullTaskManager = () => {
  ...
  const {
    state,
    setGlobalElements,
    saveData, // Destructure saveData from context
    addOfferGroup,
    editOfferGroup,
    deleteOfferGroup,
  } = useMjopContext();
  ...
  const openAddDialog = (category) => {
    ...
    selectedElementsByCategory[category].forEach((elementId) => {
      ...
      if (element) {
        ...
        elementSpaces[elementId] = state.globalSpaces.find((space) => space.id === element.spaceId)?.name || 'Onbekende Ruimte';
      }
    });
  ...
  const openEditDialog = (group) => {
    ...
    group.subtasks.forEach((subtask) => {
      ...
      elementSpaces[subtask.id] = state.globalSpaces.find((space) => space.id === subtask.spaceId)?.name || 'Onbekende Ruimte';
    });
  ...
  const addOrUpdateTaskGroup = () => {
    ...
    if (dialogMode === 'add') {
      if (periodic) {
        ...
        // Bereken het totale aantal taakgroepen op basis van periodiciteit en totale jaren
        const totalPeriods = Math.floor((parseInt(totalYears, 10) * 12) / parseInt(periodicityMonths, 10));
        ...
        for (let i = 0; i < totalPeriods; i++) {
          const periodDate = new Date(groupDate);
          periodDate.setMonth(periodDate.getMonth() + (i * parseInt(periodicityMonths, 10)));

          // Bereken de kosten met indexatie
          // Indexatie wordt toegepast per jaar. We berekenen het aantal volledige jaren sinds start.
          const fullYearsElapsed = Math.floor(periodDate.getFullYear() - groupDate.getFullYear());
          ...
          const newOfferGroup = {
            ...
            invoicePrice: "",
            offerPrice: "",
            estimatedValue: adjustedCost,
            name: `Offertegroep ${i + 1} - ${name}`,
            offerAccepted: false,
      ...
      if (periodic) {
        // Voor bewerken van periodieke taakgroepen kan dit complex zijn.
        // Afhankelijk van de behoefte kan je ervoor kiezen om bestaande groepen aan te passen of te vervangen.
        // Voor eenvoud hier, laten we aannemen dat bij bewerken geen periodieke wijziging wordt ondersteund.
        setSnackbarMessage('Periodieke taakgroepen kunnen momenteel niet bewerkt worden.');
        setSnackbarOpen(true);
        return;
      } else {
              ...
              subtasks: selectedElementIds.map((elementId) => {
                ...
                return {
                  ...
                  individualCost: assignPricesIndividually
                    ...
                    : 0,
                  endDate: new Date(groupDate),
                  offerGroupId: group.offerGroupId, // Retain existing offerGroupId
                };
              }),
            };
          }
        ...
        // Update offerGroups indien nodig (bijvoorbeeld de naam of estimatedValue)
        const correspondingOfferGroup = state.offerGroups.find(og => og.offerGroupId === taskProperties.offerGroupId);
        if (correspondingOfferGroup) {
          const updatedOfferGroup = {
            ...correspondingOfferGroup,
            name: `Offertegroep - ${name}`,
            estimatedValue: assignPricesIndividually
              ? selectedElementIds.reduce((sum, elId) => sum + parseFloat(individualCosts[elId] || 0), 0)
              : parseFloat(cost.toFixed(2)),
          ...
          editOfferGroup(updatedOfferGroup);
        }
  ...
  return (
    <Box sx={{ position: 'relative', height: '100vh', backgroundColor: '#f4f6f8', p: 3 }}>
      ...
      <TaskGroupDialog
        ...
        addOfferGroup={addOfferGroup} // Pass addOfferGroup to dialog
      />
