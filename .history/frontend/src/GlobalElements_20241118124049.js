...
const GlobalElements = ({ t }) => {
  ...
  const handleSelectElement = useCallback(
    (element, isEditMode = false, fromTable = false) => {
      ...
      setNewElement({
        ...
        gebreken: initialGebreken, // Correctly initialized
      ...
      setMaterials(Object.keys(element.gebreken || {}));
  ...
  const handleMaterialChange = useCallback(
    (e) => {
      ...
      setNewElement((prev) => ({
        ...
        // Reset gebreken when material changes
        gebreken: {
          ernstig: [],
          serieus: [],
          gering: [],
    ...
    (element, selectedType) => {
      ...
      setNewElement({
        ...
        gebreken: {
          ...
          serieus: [],
          gering: [],
        }, // Initialize gebreken with categories
        inspectionReport: {
          id: uuidv4(),
          description: "",
          inspectionDone: false,
  ...
  const handleAddCustomGebrek = useCallback(() => {
    ...
    // Validate that a severity is selected
    if (!severityCategorieën.includes(severity)) {
      setErrors({
        ...errors,
        gebreken: "Ongeldige ernst categorie geselecteerd.",
      });
      return;
    }

    addGebrek(severity, gebrekName); // Aangepast voor de nieuwe structuur

    setCustomGebrek("");
    setCustomGebrekSeverity("gering");
  }, [customGebrek, customGebrekSeverity, addGebrek, errors]);

  // Helper function to render gebreken (Updated with checkboxes)
  const renderGebreken = useCallback(() => {
    // Get gebreken based on the current structure
    const gebreken = newElement.gebreken || {};
      ...
      <Box>
        {ernstCategorieën.map((severity) => (
          <Box key={`severity-${severity}`} mb={2}>
            ...
            {gebreken[severity] && gebreken[severity].length > 0 ? (
              <List>
                {gebreken[severity].map((gebrek, index) => {
                  const isChecked = gebreken[severity].includes(gebrek);
  ...
  }, [newElement.gebreken, addGebrek, removeGebrek, theme.palette, errors]);
  ...
  const renderStepContent = useCallback(
    ...
    [activeStep, addGebrek, handleSaveElement, handleUndoAnnotation, handleDeleteAllAnnotations, newElement, errors, renderGebreken, globalSpaces, selectedImage, annotations]
  );
