...
const InspectionModal = ({
  ...
  const handleAddMistake = (reportId, gebrekName, severity) => {
    ...
    setInspectionData((prevState) => ({
      ...
      overallConditionScore: calculateConditionScore(
        updatedInspectionReport.flatMap((report) => report.mistakes),
        updatedInspectionReport.flatMap((report) => report.tasks)
      ),
    }));
  ...
  // Function to calculate overall condition score
  const calculateConditionScore = (mistakes, tasks) => {
    let score = 0;
    mistakes.forEach((mistake) => {
      ...
      score += severityScores[mistake.severity.toLowerCase()] || 0;
      score += getExtentScore(mistake.omvang) || 0;
    });
    ...
    return Math.min(score, 6); // Ensures score doesn't exceed 6
  };

  // Utility function to get extent score from percentage
  ...
  const handleSave = () => {
    ...
    setSuccessSnackbarMessage("Inspection data successfully saved.");
        ...
        >
          ...
          >
            <Typography variant="h5" color="primary">
              Inspectie Element - {inspectionData.name || "Nieuw Inspectie"}
            </Typography>
  ...
  // Utility function to calculate condition score
  const calculateConditionScore = (mistakes, tasks) => {
    let score = 0;
    mistakes.forEach((mistake) => {
      const severityScores = {
