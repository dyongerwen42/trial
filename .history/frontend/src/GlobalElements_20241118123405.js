...
const GlobalElements = ({ t }) => {
  ...
  const handleSelectElement = useCallback(
    (element, isEditMode = false, fromTable = false) => {
      ...
      if (isEditMode && element.gebreken) {
        ...
        // When adding, initialize gebreken with categorieën
      ...
      setNewElement({
        ...
        inspectionReport: Array.isArray(element.inspectionReport) && element.inspectionReport.length > 0 ? element.inspectionReport : [
          {
            id: uuidv4(),
            description: "",
    ...
    [globalSpaces, newElement.gebreken, inspectionTasks, setErrors]
  );
  ...
  const handleMaterialChange = useCallback(
    (e) => {
      ...
      setNewElement((prev) => {
        ...
        // Initialize gebreken for the selected material if not present
        if (!updatedGebreken[prev.type]) {
          updatedGebreken[prev.type] = {};
        }
        if (!updatedGebreken[prev.type][selectedMaterial]) {
          updatedGebreken[prev.type][selectedMaterial] = {
            ernstig: [],
            serieus: [],
            gering: [],
          };
  ...
  const handleTypeChangeFromSidePanel = useCallback(
    (element, selectedType) => {
      ...
      setNewElement((prev) => ({
        ...
        gebreken: {
          [selectedType]: {
            "": { // Initial material is empty string
              ernstig: [],
              serieus: [],
              gering: [],
            },
          },
        }, // Initialize gebreken with categories and materials
        unitType: "",
        quantity: "",
        inspectionReport: [
          {
  ...
  }, [newElement.type, newElement.material, newElement.name, newElement.gebreken, inspectionTasks, addGebrek, removeGebrek, theme.palette, errors]);
