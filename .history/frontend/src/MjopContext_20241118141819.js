...
const mjopReducer = (state, action) => {
  switch (action.type) {
    ...
    case "ADD_GEBREKEN":
      const { category: addCategory, gebrekenList } = action.payload;

      if (!["ernstig", "serieus", "gering"].includes(addCategory)) {
        console.warn(`Onbekende categorie: ${addCategory}`);
        return state;
      }

      const uniqueGebrekenToAdd = gebrekenList.filter(
        (g) => !state.newElement.gebreken[addCategory].includes(g)
      );

      if (uniqueGebrekenToAdd.length === 0) {
        console.warn(`Geen nieuwe gebreken om toe te voegen onder ${addCategory}.`);
        return state;
      }
      ...
      return {
        ...
        newElement: {
          ...
          gebreken: {
            ...state.newElement.gebreken,
            [addCategory]: [
              ...state.newElement.gebreken[addCategory],
              ...uniqueGebrekenToAdd,
            ],
          },
        },
    ...
    case "ADD_GEBREKEN_TO_ELEMENT":
      const { elementId, category: addElCategory, gebrekenList: addElGebrekenList } = action.payload;

      if (!["ernstig", "serieus", "gering"].includes(addElCategory)) {
        console.warn(`Onbekende categorie: ${addElCategory}`);
        return state;
      }
      ...
      return {
        ...
        globalElements: state.globalElements.map((element) => {
          if (element.id === elementId) {
            const existingGebreken = element.gebreken[addElCategory] || [];
            const uniqueGebrekenToAddEl = addElGebrekenList.filter(
              (g) => !existingGebreken.includes(g)
            );

            if (uniqueGebrekenToAddEl.length === 0) {
              console.warn(`Geen nieuwe gebreken om toe te voegen onder ${addElCategory} voor element ${elementId}.`);
              return element;
            }
