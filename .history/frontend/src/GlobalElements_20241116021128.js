...
const GlobalElements = ({ t }) => {
  ...
  const handleFileChange = async (e) => {
    ...
    // Assign unique keys to photos if necessary
    const photosWithKeys = validFileUrls.map((url) => ({
      url,
      key: uuidv4(),
    }));
    ...
    setNewElement({
      ...
      photos: [
        ...(newElement.photos || []),
        ...photosWithKeys.map((photo) => photo.url),
      ],
    });
  };
  ...
  const handleDocumentChange = async (e) => {
    ...
    // Assign unique keys to documents if necessary
    const documentsWithKeys = validFileUrls.map((url) => ({
      url,
      key: uuidv4(),
    }));
    ...
    setNewElement({
      ...
      documents: [
        ...
        ...documentsWithKeys.map((doc) => doc.url),
      ],
    });
  };
  ...
  // Remove or comment out this useEffect to prevent auto-saving
            ...
            {availableGebreken[severity].length > 0 ? (
              <List>
                {availableGebreken[severity].map((gebrek, gebrekIndex) => (
                  <ListItem
                    key={`gebrek-${severity}-${gebrek}-${gebrekIndex}`}
                    secondaryAction={
                      <IconButton
                        edge="end"
                        aria-label="delete"
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} md={6}>
              <FormControl fullWidth variant="outlined" sx={{ mb: 2 }}>
                ...
                >
                  {Object.keys(taken.onderhoudstaken).map((category, categoryIndex) => (
                    <MenuItem
            ...
            <Grid item xs={12} md={6}>
              ...
              >
                ...
                >
                  ...
                  {globalSpaces.map((space, spaceIndex) => (
                    <MenuItem
                      key={`space-${space.id}-${spaceIndex}`}
                      value={space.id}
                    >
                      {space.name}
                    </MenuItem>
        ...
        >
          ...
          <List>
            {filteredElements.map((element, index) => {
              ...
              // Construct a unique key by combining element.id and index
              const uniqueKey = `element-${element.id || `no-id-${index}`}`;
                  ...
                  {isExpanded && (
                    ...
                    >
                      {Object.keys(
                        ...
                        <ListItem
                          button
                          key={`type-${type}-${typeIndex}-${uniqueKey}`}
                          onClick={() =>
          ...
          <table {...getTableProps()} style={{ width: "100%" }}>
            ...
            <tbody {...getTableBodyProps()}>
              {rows.map((row, rowIndex) => {
                ...
                const rowKey = `table-row-${row.original.id || `row-${rowIndex}`}`;
                ...
                return (
                  ...
                  >
                    {row.cells.map((cell, cellIndex) => {
                      // Ensure each cell has a unique key
                      const cellKey = `table-cell-${rowKey}-${cell.column.id}-${cellIndex}`;
                      return (
                        <td
                          {...cell.getCellProps()}
                          key={cellKey}
