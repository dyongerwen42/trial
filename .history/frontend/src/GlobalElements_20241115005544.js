...
const GlobalElements = ({ t }) => {
  ...
  const handleMaterialChange = (e) => {
    ...
    const gebrekenList = extractGebreken(
      newElement.name,
      newElement.type,
      selectedMaterial,
      inspectionTasks
    ...
    // Zorg ervoor dat gebreken correct wordt toegewezen onder de juiste categorie, materiaal en ernst
    const updatedGebreken = {
      ...newElement.gebreken,
      [newElement.type]: gebrekenList,
    };
  ...
  const handleSelectGebrek = (category, material, severityCategory, gebrekName) => {
    console.log("Handle Select Gebrek:", { category, material, severityCategory, gebrekName });
    const isSelected =
      newElement.gebreken[category] &&
      newElement.gebreken[category][material] &&
      newElement.gebreken[category][material][severityCategory] &&
      newElement.gebreken[category][material][severityCategory].includes(gebrekName);
    ...
    if (isSelected) {
      ...
      addGebrek(category, material, severityCategory, gebrekName);
    }
  };

  // Functie om de ernst van een gebrek te wijzigen
  const handleChangeSeverity = (category, material, currentSeverity, gebrekName, newSeverity) => {
    console.log("Handle Change Severity:", { category, material, currentSeverity, gebrekName, newSeverity });
    if (currentSeverity === newSeverity) return; // Geen verandering

    // Verwijder het gebrek uit de huidige ernstcategorie
    removeGebrek(category, material, currentSeverity, gebrekName);
    // Voeg het gebrek toe aan de nieuwe ernstcategorie
    addGebrek(category, material, newSeverity, gebrekName);
  };
  ...
  const handleAddCustomGebrek = () => {
    ...
    addGebrek(category, material, severityCategory, customGebrek.trim());
  ...
  const renderStepContent = (step) => {
    switch (step) {
            ...
            <Grid item xs={12} md={6}>
              <Box display="flex" alignItems="center" sx={{ mb: 2 }}>
                <TextField
                  ...
                  onChange={(e) =>
                    setNewElement({ ...newElement, quantity: e.target.value })
                  }
                ...
                >
                  ...
                  <Select
                    ...
                    onChange={(e) =>
                      setNewElement({ ...newElement, unitType: e.target.value })
                    }
                    label="Eenheid"
                  >
                    <MenuItem value="">
      ...
      case 3:
        // Gebreken & Inspecties
        // We tonen gebreken gebaseerd op de geselecteerde element, type, materiaal en ernst
        const currentGebreken = newElement.gebreken || {};
        ...
        return (
          <Grid container spacing={2}>
            <Grid item xs={12}>
              ...
              <Divider sx={{ mb: 2 }} />
              {Object.keys(currentGebreken).map((category, catIndex) => (
                <Box key={catIndex} mb={2}>
                  <Typography variant="subtitle1" gutterBottom>
                    {category.charAt(0).toUpperCase() + category.slice(1)}
                  </Typography>
                  {Object.keys(currentGebreken[category]).map((material, matIndex) => (
                    <Box key={material} mb={1} pl={2}>
                      <Typography variant="subtitle2">
                        Materiaal: {material.charAt(0).toUpperCase() + material.slice(1)}
                      </Typography>
                      {Object.keys(currentGebreken[category][material]).map((severity) => (
                        <Box key={severity} mb={1} pl={2}>
                          <Typography variant="body2" sx={{ fontWeight: 'bold' }}>
                            Ernst: {severity.charAt(0).toUpperCase() + severity.slice(1)}
                          </Typography>
                          <Grid container spacing={1}>
                            {Array.isArray(currentGebreken[category][material][severity]) &&
                              currentGebreken[category][material][severity].map((gebrek) => (
                                    ...
                                    <FormControlLabel
                                      control={
                                        <Checkbox
                                          ...
                                          onChange={() =>
                                            handleSelectGebrek(category, material, severity, gebrek)
                                          }
                                        />
                                      }
                                      label={gebrek}
              ...
              {/* Display Custom Gebreken */}
              {currentGebreken.customCategory && (
                <Box mt={4}>
                  <Typography variant="subtitle1" gutterBottom>
                    Aangepaste Gebreken
                  </Typography>
                  <Grid container spacing={1}>
                    {currentGebreken.customCategory.map((gebrek) => (
                      <Grid item xs={12} sm={6} md={4} key={gebrek.name}>
                        <Box display="flex" alignItems="center">
                          <FormControlLabel
                            control={
                              <Checkbox
                                checked={true}
                                onChange={() =>
                                  handleSelectGebrek("customCategory", "customMaterial", gebrek.severity, gebrek.name)
                                }
                              />
                            }
                            label={gebrek.name}
                          ...
                          >
                            <Select
                              value={gebrek.severity}
                              onChange={(e) =>
                                handleChangeSeverity("customCategory", "customMaterial", gebrek.severity, gebrek.name, e.target.value)
                              }
                            >
                              <MenuItem value="ernstig">Ernstig</MenuItem>
                              <MenuItem value="serieus">Serieus</MenuItem>
