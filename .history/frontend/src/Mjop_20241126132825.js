  ...
  const addOrUpdateTaskGroup = () => {
    ...
    if (dialogMode === 'add') {
      if (periodic) {
        ...
        const totalPeriods = Math.floor((totalYears * 12) / periodicityMonths);
        ...
        for (let i = 0; i < totalPeriods; i++) {
          ...
          periodDate.setMonth(periodDate.getMonth() + (i * periodicityMonths));

          // Bereken de kosten met indexatie
          const adjustedCost = indexation ? cost * Math.pow((1 + indexRate), Math.floor(i / (12 / periodicityMonths))) : cost;
