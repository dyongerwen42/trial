...
const GlobalElements = ({ t }) => {
  ...
  const handleSaveElement = async () => {
    ...
    if (
      ...
      newErrors.gebreken = newErrors.gebreken
        ? `${newErrors.gebreken} En minimaal één serieus gebrek is vereist`
        : "Minimaal één serieus gebrek is vereist";
    }

    if (Object.keys(newErrors).length > 0) {
      setErrors(newErrors);
      const invalidSteps = [];
      if (newErrors.name || newErrors.interval || newErrors.description || newErrors.categories)
        invalidSteps.push(0);
