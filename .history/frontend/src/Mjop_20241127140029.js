...
const FullTaskManager = () => {
  const {
    state,
    isDataLoaded, // Destructure isDataLoaded from context
    setGlobalElements,
    saveData,
    addOfferGroup,
    editOfferGroup,
  ...
  const addOrUpdateTaskGroup = useCallback(() => {
    ...
    if (dialogMode === 'add') {
      if (periodic) {
        ...
        // This else block handles the non-periodic task group addition
        // The logic is similar to the periodic case but without loops
  ...
  // Implementing the edit logic to update existing task groups and offer groups
  const editTaskGroup = useCallback(() => {
    ...
    selectedElementIds.forEach((elementId) => {
      ...
      const updatedOfferGroup = {
        ...state.globalOfferGroups.find(og => og.offerGroupId === existingSubtask.offerGroupId),
        estimatedValue: assignPricesIndividually
          ? parseFloat((individualCosts[elementId] || 0).toFixed(2))
          : parseFloat(cost.toFixed(2)),
        name: `Offertegroep - ${element.name}`,
      ...
      if (updatedTasksByElementId[el.id]) {
        return {
          ...
          tasks: el.tasks.map((task) =>
            task.groupId === id ? updatedTasksByElementId[el.id].find(t => t.id === task.id) || task : task
          ),
        };
      }
      return el;
    ...
    setTaskGroups((prevGroups) =>
      prevGroups.map((tg) => (tg.id === id ? updatedTaskGroups.find(utg => utg.id === tg.id) || tg : tg))
    );
  ...
  const handleDeleteTaskGroup = () => {
    ...
    // Remove the corresponding offerGroup
    if (offerGroupId) {
  ...
  if (!isDataLoaded) { // Use isDataLoaded directly
    return (
