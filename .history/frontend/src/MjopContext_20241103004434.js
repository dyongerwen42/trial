...
const mjopReducer = (state, action) => {
  ...
  switch (action.type) {
    ...
    case 'SET_TAB_ERRORS':
      console.log('Setting tab errors with payload:', action.payload);
      return { ...state, tabErrors: action.payload };
    case 'SET_ERRORS':
      console.log('Setting errors with payload:', action.payload);
      return { ...state, errors: action.payload };
    case 'SET_ERROR_DIALOG':
      console.log('Setting error dialog with payload:', action.payload);
      return { ...state, openErrorDialog: action.payload.open, errorMessage: action.payload.message };
    case 'SET_SAVING':
      console.log('Setting saving state with payload:', action.payload);
      return { ...state, isSaving: action.payload };
    case 'SET_SUCCESS':
      console.log('Setting success message with payload:', action.payload);
      return { ...state, success: action.payload };
    case 'ADD_ELEMENT':
      console.log('Adding element with payload:', action.payload);
      return { ...state, globalElements: [...state.globalElements, action.payload] };
    case 'EDIT_ELEMENT':
      console.log('Editing element with payload:', action.payload);
    ...
    case 'ADD_SPACE':
      console.log('Adding space with payload:', action.payload);
      return { ...state, globalSpaces: [...state.globalSpaces, action.payload] };
    case 'EDIT_SPACE':
      console.log('Editing space with payload:', action.payload);
...
export const MjopProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (id && id !== mjopId) {
      ...
      console.log('mjopId is unchanged or null, no update required.');
    }
  ...
  useEffect(() => {
    ...
    if (mjopId) {
      ...
      console.log("No initializeData call since mjopId is null or invalid.");
    }
  }, [mjopId]);

  const handleValidation = () => {
    console.log("Running validation with current state:", state);
  ...
  const saveData = async () => {
    console.log("Attempting to save data with state:", state);
    if (!handleValidation()) {
