  ...
  const handleAddTask = useCallback(() => {
    ...
    selectedElements.forEach((element) => {
      ...
      selectedTasks.forEach((task, taskIndex) => {
        ...
        // Use the adjusted dates or fallback to defaults
        const adjustedEndDate = task.adjustments.endDate
          ? new Date(task.adjustments.endDate)
          : new Date(customTask.ultimateDate);
        const adjustedWorkDate = task.adjustments.workDate
          ? new Date(task.adjustments.workDate)
          : null;
        const adjustedStartDate = task.adjustments.startDate
          ? new Date(task.adjustments.startDate)
          : new Date();
        ...
        const newTask = {
          ...
          ultimateDate: adjustedEndDate.toISOString(),
          estimatedPrice: currentEstimatedPrice.toFixed(2),
          planned: {
            workDate: adjustedWorkDate ? adjustedWorkDate.toISOString() : null,
            startDate: offerAccepted ? adjustedStartDate.toISOString() : null,
              ...
              selectedTasks.map((task, index) => (
                <Accordion key={index} defaultExpanded>
                  ...
                  <AccordionDetails>
                    ...
                    {/* Date Picker for Werkdatum */}
                    <LocalizationProvider dateAdapter={AdapterDateFns}>
                      <DatePicker
                        label="Werkdatum"
                        value={task.adjustments.workDate || null}
                    ...
                    {/* Date Picker for Einddatum */}
                    <LocalizationProvider dateAdapter={AdapterDateFns}>
                      <DatePicker
                        label="Einddatum"
                        value={task.adjustments.endDate || null}
                    ...
                    <FormControlLabel
                      ...
                      label="Werkdatum direct instellen als einddatum"
                    />
                  </AccordionDetails>
                </Accordion>
              ))
                    ...
                    <TableBody>
                      {selectedElements.map((element) => {
                        ...
                        return selectedTasks.map((task, taskIndex) => {
                          ...
                          // Use adjusted dates or default values
                          const adjustedEndDate = task.adjustments.endDate
                            ? new Date(task.adjustments.endDate)
                            : new Date(customTask.ultimateDate);
                          const adjustedWorkDate = task.adjustments.workDate
