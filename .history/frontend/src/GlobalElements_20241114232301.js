...
import GebrekenSelectie from "./GebrekenSelectie"; // Importeer het nieuwe component
...
const extractGebreken = (elementName, type, material, data) => {
  ...
  if (
    ...
    const gebrekenCopy = JSON.parse(JSON.stringify(element.gebreken[type][material]));
    // Initialize selectedGebreken
    Object.keys(gebrekenCopy).forEach((ernst) => {
      gebrekenCopy[ernst].selectedGebreken = [];
    });
    return gebrekenCopy;
  }
  return {};
...
const GlobalElements = ({ t }) => {
  ...
  const handleTypeChange = (e) => {
    ...
    setMaterials(materials);
    const newGebreken = extractGebreken(
      newElement.name,
      selectedType,
      '', // Initially no material
      inspectionTasks
    );
    setNewElement({
      ...newElement,
  ...
  // Handle selecting en deselecting gebreken
  const handleSelectGebrek = (category, subcategory, ernst, gebrek) => {
    const updatedGebreken = { ...newElement.gebreken };

    // Initialize if not present
    if (
      !updatedGebreken[category] ||
      !updatedGebreken[category][subcategory] ||
      !updatedGebreken[category][subcategory][ernst]
    ) {
      return;
    }

    const selectedGebreken = updatedGebreken[category][subcategory][ernst].selectedGebreken || [];

    if (selectedGebreken.includes(gebrek)) {
      // Deselect
      updatedGebreken[category][subcategory][ernst].selectedGebreken = selectedGebreken.filter(
        (g) => g !== gebrek
      );
    } else {
      // Select
      updatedGebreken[category][subcategory][ernst].selectedGebreken = [...selectedGebreken, gebrek];
    }

    updateGebreken(category, updatedGebreken[category]);
  };

  // Handle adding custom gebrek
  const handleAddCustomGebrek = (customGebrek) => {
    // Voeg custom gebrek toe aan een speciale categorie, bijvoorbeeld 'custom'
    const updatedGebreken = { ...newElement.gebreken };

    if (!updatedGebreken['custom']) {
      updatedGebreken['custom'] = {
        'customSubcategory': {
          'customErnst': {
            'ernstig': [],
            'serieus': [],
    ...
    // Voeg aan een specifieke ernst toe, bijvoorbeeld 'gering'
    updatedGebreken['custom']['customSubcategory']['gering'].push(customGebrek);
              ...
              <Box display="flex" alignItems="center" sx={{ mb: 2 }}>
                ...
                >
                  ...
                  >
                    ...
                    <MenuItem value="vierkante_meter">
                      Vierkante meter
                    </MenuItem>
                    <MenuItem value="strekkende_meter">
                      Strekkende meter
                    </MenuItem>
                  ...
                  onChange={(e) => {
                    ...
                    if (selectedSpace) {
                      const imageWithAnnotations =
                        selectedSpace.image.startsWith("http")
                          ? selectedSpace.image
        ...
        return (
          <Grid container spacing={2}>
            <Grid item xs={12}>
              ...
              <GebrekenSelectie
                gebreken={newElement.gebreken}
                onSelectGebrek={handleSelectGebrek}
                onAddCustomGebrek={handleAddCustomGebrek}
              />
            </Grid>
            {selectedImage && (
              <Grid item xs={12}>
        ...
        return (
          <Grid container spacing={2}>
            ...
            {newElement.photos.length > 0 && (
              <Grid item xs={12}>
                <Typography variant="body1">
                  <strong>Geüploade Foto's:</strong> {newElement.photos.length}{" "}
                  bestand(en)
                </Typography>
              </Grid>
            )}
