...
const FullTaskManager = () => {
    ...
    return globalElements.reduce((acc, element) => {
      ...
      elementCategories.forEach((category) => {
        ...
        acc[category].push({
          ...element,
          spaceName:
            globalSpaces.find((space) => space.id === element.spaceId)?.name || 'Onbekende Ruimte',
        });
      });
      return acc;
    }, {});
  ...
  const addTasksToElements = () => {
    ...
    const updatedGlobalElements = globalElements.map((element) => {
      if (selectedElementIds.includes(element.id)) {
        const elementCost = assignPricesIndividually
        ...
        const newTask = {
          ...
          endDate: new Date(year, 0, 1).toISOString(),
          cost: elementCost,
          isGrouped: true,
  ...
  const tasksByYear = useMemo(() => {
    ...
    const tasksByYear = tasks.reduce((acc, task) => {
      const taskYear = new Date(task.endDate).getFullYear();
      if (!acc[taskYear]) {
        acc[taskYear] = [];
      }
      acc[taskYear].push(task);
      return acc;
    }, {});
    return tasksByYear;
  }, [globalElements]);
