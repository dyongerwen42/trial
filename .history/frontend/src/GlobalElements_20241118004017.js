...
const GlobalElements = ({ t }) => {
  ...
  const handleMaterialChange = (e) => {
    ...
    if (
      ...
      if (
        Object.keys(updatedGebreken[newElement.type][newElement.material]).length ===
        0
      ) {
  ...
  const handleTaskSubmit = () => {
    if (!currentTask.description) {
      setErrors({ ...errors, task: "Beschrijving van de taak is verplicht." });
      return;
    }

    let updatedInspectionReport = [...newElement.inspectionReport];
    if (isEditingTask) {
      updatedInspectionReport = updatedInspectionReport.map((report) => ({
        ...report,
      ...
      const newTask = { ...currentTask, id: uuidv4() };
      updatedInspectionReport = updatedInspectionReport.map((report) => ({
        ...report,
          ...
          <Grid container spacing={2}>
            <Grid item xs={12}>
              <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                <Typography variant="h6">Inspectietaken</Typography>
                <Button
                  variant="contained"
                  color="primary"
              ...
              {newElement.inspectionReport.map((report, reportIndex) => (
                <Box key={`inspection-report-${report.id}-${reportIndex}`} mb={4}>
                  <Typography variant="subtitle1" sx={{ fontWeight: "bold" }}>
                    Inspectierapport #{reportIndex + 1}
                  </Typography>
                  <List>
                          ...
                          <Box>
                            ...
                            <IconButton
                              ...
                              onClick={() => {
                                ...
                                const updatedReport = newElement.inspectionReport.map((r) =>
                                  r.id === report.id
                                    ? {
                                        ...r,
      ...
      case 5:
        ...
        return (
          <Grid container spacing={2}>
            ...
            {newElement.inspectionReport.map((report, reportIndex) => (
              <Grid item xs={12} key={`review-inspection-${report.id}-${reportIndex}`}>
                <Typography variant="h6">
                  Inspectierapport #{reportIndex + 1}
                </Typography>
                {report.tasks.length > 0 ? (
                  <List>
                    {report.tasks.map((task) => (
                      <ListItem key={`review-task-${task.id}`}>
                        <ListItemText
                          primary={task.description}
                          secondary={`Planned: ${task.planned ? "Yes" : "No"}`}
                        />
  ...
  return (
    <Box display="flex" height="100vh">
      ...
      {/* Inspectietaken Dialog */}
      <Dialog open={openTaskDialog} onClose={handleCloseTaskDialog}>
        <DialogTitle>{isEditingTask ? "Bewerk Inspectietaak" : "Voeg Inspectietaak Toe"}</DialogTitle>
        <DialogContent>
          <TextField
            fullWidth
            variant="outlined"
            label="Beschrijving van de taak"
          ...
          <FormControlLabel
            control={
              <Checkbox
                checked={currentTask.planned}
                onChange={(e) =>
                  setCurrentTask({ ...currentTask, planned: e.target.checked })
                }
              />
            }
            label="Gepland"
          ...
          {currentTask.planned && (
            <Box sx={{ mt: 2 }}>
              <TextField
                ...
                value={currentTask.plannedData.workDate}
                onChange={(e) =>
                  setCurrentTask({
                    ...currentTask,
                    plannedData: {
                      ...currentTask.plannedData,
              ...
              <TextField
                ...
                type="number"
                value={currentTask.plannedData.estimatedPrice}
                onChange={(e) =>
                  setCurrentTask({
                    ...currentTask,
                    plannedData: {
              ...
              <TextField
                ...
                multiline
                rows={3}
                value={currentTask.plannedData.comment}
                onChange={(e) =>
                  setCurrentTask({
                    ...currentTask,
                    plannedData: {
              ...
              {/* Voeg hier eventueel functionaliteit toe voor het uploaden van bestanden */}
            </Box>
          )}
        </DialogContent>
        <DialogActions>
          ...
          <Button variant="contained" onClick={handleTaskSubmit}>
            {isEditingTask ? "Bewerken" : "Toevoegen"}
          </Button>
        </DialogActions>
      </Dialog>
    </Box>
