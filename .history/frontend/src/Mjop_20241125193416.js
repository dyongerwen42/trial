        ...
        <Grid item xs={12} md={4}>
          ...
          >
            ...
            {Object.entries(categorizedElements).map(([category, spaces]) => {
              ...
              return (
                <Accordion key={category} sx={{ mb: 1, boxShadow: 'none', border: 'none' }}>
                  <AccordionSummary
                  ...
                  <AccordionDetails sx={{ p: 0 }}>
                    ...
                    {Object.entries(spaces).map(([spaceName, elements]) => (
                      <Box key={spaceName} sx={{ mb: 1, px: 1 }}>
                        {/* Space Header */}
                        <Typography variant="subtitle2" sx={{ fontWeight: 500, fontSize: '0.85rem', mb: 0.5 }}>
                          {spaceName}
                        </Typography>
                              ...
                              <CardContent sx={{ p: 0, pt: 0.5 }}>
                                ...
                                {element.photos && element.photos.length > 0 && (
                                  <Box display="flex" flexWrap="wrap" gap={0.5} mt={0.5}>
                                    {element.photos.slice(0, 2).map((photo, index) => (
                                      <img
                                        key={index}
                                        src={photo.replace('\\\\', '/')}
          ...
          {currentTaskGroupInfo ? (
            <Box>
              ...
              {currentTaskGroupInfo.subtasks.map((element) => (
                <Card key={element.id} variant="outlined" sx={{ mb: 1, borderRadius: 1 }}>
                  <CardHeader
                    title={
                    ...
                    action={
                      <Tooltip title="Bekijk details" arrow>
                        <IconButton aria-label="info" onClick={() => handleOpenInfoDialog(element)} size="small">
