...
const InspectionModal = ({
  ...
  const {
    ...
    addGebrekFunc, // Zorg ervoor dat deze functie is toegevoegd aan de context
  } = useMjopContext();
  ...
  // Function to handle image uploads for a task
  ...
  // Function to clear an image from a task
  ...
  // Function to handle document uploads for a task
  ...
  // Function to clear a document from a task
  ...
  // Handler to save the inspection data and tasks
  const handleSave = () => {
    ...
    const updatedElements = globalElements.map((element) => {
      if (element.id === currentElementId) {
        ...
        // Beheer taken op elementniveau: vervang de bestaande taken met de bijgewerkte taken
        return {
          ...element,
                        ...
                        <Autocomplete
                          ...
                          onChange={(event, newValue) => {
                            if (typeof newValue === 'string') {
                              ...
                              // Voeg nieuwe categorie toe aan de context
                              addGebrekFunc(mistake.severity, newValue);
                            } else if (newValue && newValue.inputValue) {
                              handleMistakeChange(index, "category", newValue.inputValue);
                              // Voeg nieuwe categorie toe aan de context
                              addGebrekFunc(mistake.severity, newValue.inputValue);
                            } else {
                              handleMistakeChange(index, "category", newValue);
                            }
                          }}
