...
const TaskCreationForm = () => {
  ...
  const handleAddTask = useCallback(() => {
    ...
    const overlappingTasks = existingTasks.filter((existingTask) =>
      selectedTasks.some(
        (selectedTask) =>
          (selectedTask.adjustments.name || selectedTask.beschrijving) === existingTask.name &&
          (selectedTask.adjustments.description || selectedTask.toelichting) ===
            existingTask.description
      )
    );

    if (overlappingTasks.length > 0) {
      const overlappingTaskNames = overlappingTasks.map((task) => task.name).join(', ');
      alert(
        `De volgende taken zijn al gepland voor de geselecteerde elementen: ${overlappingTaskNames}`
      );
      // Optionally, you can prevent adding duplicate tasks
      return;
    }
