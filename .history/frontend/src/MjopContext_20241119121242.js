...
const InspectionModal = ({
  ...
  const { state, setGlobalElements, saveData, addGebrekToElement, removeGebrekFromElement } = useMjopContext();
  ...
  const handleAddMistake = (category, severity) => {
    ...
    addGebrekToElement(currentElementId, category, newMistake);
  };
  ...
  const handleRemoveMistake = (index) => {
    ...
    removeGebrekFromElement(currentElementId, removedMistake.category, removedMistake);
  };
  ...
  const calculateConditionScore = (mistakes) => {
    ...
    const validMistakes = mistakes.filter(
      (mistake) =>
        ...
        mistake.intensity &&
        mistake.omvang &&
        severityMapping[mistake.severity.toLowerCase()]
    );
    ...
    validMistakes.forEach((mistake) => {
      ...
      // Validate intensity and extent
      if (
        isNaN(intensityScore) ||
        intensityScore < 1 ||
        intensityScore > 3 ||
        ...
        console.warn(
          `Invalid intensity or extent for mistake ID ${mistake.id}. Skipping this mistake.`
        );
        return; // Skip invalid mistakes
      }
    ...
    Object.keys(situation2Groups).forEach((key) => {
      ...
      if (defects.length > 1) {
        ...
        // Sum extents
        let totalExtentScore = defects.reduce(
          (sum, defect) => sum + defect.extentScore,
          0
        );
        if (totalExtentScore > 5) {
          totalExtentScore = 5; // Cap at 5 (extentScore 5 corresponds to 70%+)
        }
    ...
    situation3Defects.forEach((defect) => {
      const { severity, intensityScore, replacementValue, extentScore } = defect;
      const conditionScore = getConditionScore(
        severity,
        intensityScore,
        extentScore
  ...
  const tabLabelWithError = (label, errorKey) => (
    <Badge
      variant="dot"
      color="error"
      invisible={true} // Always invisible since validations are removed
    >
      {label}
    </Badge>
  );
    ...
    const updatedElements = globalElements.map((element) => {
      if (element.id === currentElementId) {
        ...
        const updatedInspectionReport = inspectionExists
          ...
          : [...element.inspectionReport, { ...inspectionData, id: uuidv4() }];
