...
const FullTaskManager = () => {
  ...
  const selectAllTasks = () => {
    setSelectedTasks(globalElements.flatMap((element) => element.tasks).map((task) => task.id));
  };
    ...
    <Grid container spacing={2} sx={{ height: "100vh", backgroundColor: "#f5f5f5" }}>
      ...
      <Dialog open={isTaskDialogOpen} onClose={() => setIsTaskDialogOpen(false)}>
        ...
        <DialogContent>
          ...
          <FormControl fullWidth margin="dense">
            <InputLabel>Periodic</InputLabel>
            <Select
              value={newTaskDetails.periodic}
              onChange={(e) => handleFieldChange("periodic", e.target.value === "true")}
            >
