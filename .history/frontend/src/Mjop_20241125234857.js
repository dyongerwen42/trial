...
// Import the newly created dialog components
...
const FullTaskManager = () => {
  ...
  const openEditDialog = (group) => {
    ...
    setTaskProperties({
      ...
      periodic: group.periodic || 'None',
      indexation: group.indexation || false,
      ...
      individualDates: group.subtasks.reduce((acc, subtask) => {
        acc[subtask.id] = subtask.endDate ? new Date(subtask.endDate) : new Date(group.groupDate);
        return acc;
      }, {}),
  ...
  const addOrUpdateTaskGroup = () => {
    ...
    const numberOfPeriods = 12; // For example, create 12 periods

    if (dialogMode === 'add') {
      // Handle Periodic Task Groups
      if (periodic !== 'None') {
        for (let i = 0; i < numberOfPeriods; i++) {
          const newGroupDate = new Date(groupDate);
          if (periodic === 'Monthly') {
            newGroupDate.setMonth(newGroupDate.getMonth() + i);
          } else if (periodic === 'Quarterly') {
            newGroupDate.setMonth(newGroupDate.getMonth() + i * 3);
          } else if (periodic === 'Yearly') {
            newGroupDate.setFullYear(newGroupDate.getFullYear() + i);
          }

          const groupId = uuidv4();
          const newTaskGroup = {
            id: groupId,
            name: `${name} - ${newGroupDate.getFullYear()}`,
            ...
            periodic,
            indexation,
            indexationRate: Number(indexationRate) || 0,
            subtasks: selectedElementIds.map((elementId) => {
              const element = globalElements.find((el) => el.id === elementId);
              return {
                id: uuidv4(),
                name: elementNames[elementId],
                description: element.description || '',
                spaceId: element.spaceId,
                endDate: new Date(individualDates[elementId] || newGroupDate),
          ...
          // Update tasks in globalElements
          const updatedGlobalElements = globalElements.map((element) => {
            if (selectedElementIds.includes(element.id)) {
              const newTasks = (element.tasks || []).concat(newTaskGroup.subtasks);
        ...
        setSnackbarMessage(`${numberOfPeriods} Periodieke taakgroepen succesvol toegevoegd!`);
      } else {
        // Non-periodic Task Group
        const groupId = uuidv4();
        const newTaskGroup = {
          id: groupId,
          name,
          ...
          subtasks: selectedElementIds.map((elementId) => {
            ...
            return {
              ...
              description: element.description || '',
              spaceId: element.spaceId,
              endDate: new Date(individualDates[elementId] || groupDate),
              individualCost: assignPricesIndividually
                ? Number(individualCosts[elementId] || 0)
                : Number(cost),
              isGrouped: true,
        ...
        if (group.id === id) {
          return {
            ...
            subtasks: selectedElementIds.map((elementId) => {
              const element = globalElements.find((el) => el.id === elementId);
              return {
                id: group.subtasks.find((sub) => sub.id === elementId)?.id || uuidv4(),
                name: elementNames[elementId],
                description: element.description || '',
                spaceId: element.spaceId,
                endDate: new Date(individualDates[elementId] || group.groupDate),
                individualCost: assignPricesIndividually
                  ? Number(individualCosts[elementId] || 0)
                  : Number(cost),
                isGrouped: true,
      ...
      const updatedGlobalElements = globalElements.map((element) => {
        if (selectedElementIds.includes(element.id)) {
          const updatedTasks = (element.tasks || []).map((task) => {
            const updatedGroup = updatedTaskGroups.find((grp) => grp.id === task.groupId);
            if (updatedGroup) {
              const updatedSubtask = updatedGroup.subtasks.find((sub) => sub.id === task.id);
              if (updatedSubtask) {
