...
import {
  ...
  Save as SaveIcon, // Import SaveIcon here
...
const GlobalSpaces = () => {
  ...
  // New state variables for saving feedback
  const [isSaving, setIsSaving] = useState(false); // Indicates if save is in progress
  ...
  useEffect(() => {
    const previews = newSpace?.photos?.map((file) => {
      if (typeof file === 'string') {
        return `http://34.34.100.96:5000/${file.replace(/\\/g, '/')}`;
      ...
      },
      {
        ...
        Cell: ({ row }) => (
          <Box display="flex" alignItems="center">
            <IconButton onClick={() => handleOpenModal(row.original.photos)} aria-label="open photos modal">
              <AddPhotoAlternateIcon />
            </IconButton>
            <IconButton onClick={() => handleOpenModal(row.original.documents)} aria-label="open documents modal">
              <InsertDriveFileIcon />
            </IconButton>
            <IconButton onClick={() => handleOpenImageModal(row.original.image, row.original.annotations)} aria-label="view image with annotations">
              <VisibilityIcon />
            </IconButton>
          </Box>
        ),
          ...
          <Box display="flex" justifyContent="space-around">
            <IconButton
              onClick={() => {
                ...
                setSelectedImage(formattedImage ? `http://34.34.100.96:5000/${formattedImage}` : null);
  ...
  const handleSaveSpace = async () => {
    ...
    try {
      ...
      // Directly save to backend after adding/editing
      await saveData();
  ...
  // Removed the useEffect that auto-saves on globalSpaces changes
  /*
  useEffect(() => {
    if (state.globalSpaces.length > 0) {
  ...
  }, [state.globalSpaces]); // Trigger on changes to globalSpaces
  */
  ...
  const handleImageChange = async (e) => {
    ...
    if (uploadedFile) {
      const imageUrl = `http://34.34.100.96:5000/${uploadedFile.replace(/\\/g, '/')}`;
  ...
  const handleExistingDrawingSelect = (e) => {
    const selectedImage = e.target.value.startsWith('http') ? e.target.value : `http://34.34.100.96:5000/${e.target.value.replace(/\\/g, '/')}`;
  ...
  const handleClearImage = (index) => {
    ...
    setImagePreviews(updatedPhotos.map((file) => (typeof file === 'string' ? `http://34.34.100.96:5000/${file.replace(/\\/g, '/')}` : URL.createObjectURL(file))));
  };
  ...
  const handleClearDocument = (index) => {
    ...
    setDocumentPreviews(updatedDocuments.map((file) => (typeof file === 'string' ? `http://34.34.100.96:5000/${file.replace(/\\/g, '/')}` : URL.createObjectURL(file))));
  };
            ...
            {documentPreviews.length > 0 && (
              <Box mt={2} display="flex" flexWrap="wrap">
                {documentPreviews.map((src, index) => (
                  <Box key={index} position="relative" m={1}>
                    <a href={src} target="_blank" rel="noopener noreferrer" aria-label="view document">
        ...
        return (
          <FormControl fullWidth variant="outlined">
            ...
            <Select
              ...
              sx={{ mb: 2 }}
              aria-label="select existing drawing"
            >
              <MenuItem value="">
                <em>Selecteer bouwtekening</em>
              </MenuItem>
            ...
            <Button
              ...
              startIcon={<AddPhotoAlternateIcon />}
              sx={{ mb: 2, fontSize: '0.875rem', maxWidth: '200px' }}
              aria-label="upload existing drawing"
            >
              Bouwtekening
              <input
                type="file"
      ...
      >
        ...
        <Box display="flex" alignItems="center" sx={{ mb: 2 }}>
          <Button
            variant="contained"
            color="primary"
            startIcon={<SaveIcon />} // Ensure SaveIcon is imported
            onClick={handleSaveSpace}
            disabled={isSaving}
            sx={{ textTransform: 'none' }}
            aria-label="save spaces"
          >
            Opslaan
          </Button>
          ...
          <Card sx={{ mb: 4, p: 3, borderRadius: 2, boxShadow: 3 }}>
            <CardContent>
              ...
              <Box display="flex" justifyContent="space-between" mt={4}>
                {activeStep !== 0 && (
                  <Button onClick={handleBack} variant="contained" color="secondary" sx={{ textTransform: 'none' }} aria-label="go back">
                    Terug
                  </Button>
                )}
                <Box display="flex" gap={2}>
                  <Button variant="contained" color="secondary" onClick={resetNewSpace} sx={{ textTransform: 'none' }} aria-label="reset form">
                    Reset
                  </Button>
            ...
            <Box sx={modalStyle}>
              ...
              <Box display="flex" flexWrap="wrap" mt={2}>
                {selectedFiles.map((file, index) => {
                  ...
                  return (
                    <Box key={index} position="relative" m={1}>
                      <a href={file} download target="_blank" rel="noopener noreferrer" aria-label={`download ${file.split('/').pop()}`}>
                        {icon}
                      </a>
                      <Box display="flex" justifyContent="center" mt={1}>
                        <IconButton
              ...
              <Button onClick={handleCloseModal} sx={{ mt: 2 }} variant="contained" aria-label="close modal">
                Sluiten
              </Button>
            </Box>
          </Fade>
          ...
          <Fade in={openImageModal}>
            <Box sx={modalStyle}>
              ...
              <Box display="flex" justifyContent="flex-end" mt={2}>
                <Button onClick={handleCloseImageModal} variant="contained" aria-label="close image modal">
                  Sluiten
                </Button>
              </Box>
            </Box>
