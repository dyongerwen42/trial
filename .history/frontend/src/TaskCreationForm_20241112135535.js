...
const TaskCreationForm = () => {
  ...
  const handleElementChange = useCallback(
    ...
    [globalElements, multiAssign, selectedSpace, selectedTaskType, globalSpaces, tasksData]
  );
  ...
  const handleAddTask = useCallback(() => {
    ...
    selectedElements.forEach((element) => {
      ...
      selectedTasks.forEach((task, taskIndex) => {
        ...
        if (offerAccepted) {
          newUltimateDate = addDays(newStartDate, parseInt(customTask.durationDays) || 0);
        }
    ...
    switch (step) {
      case 0:
        ...
        const uniqueElementNames = useMemo(() => {
          return [...new Set(filteredElements.map((el) => el.name))];
        }, [filteredElements]);
