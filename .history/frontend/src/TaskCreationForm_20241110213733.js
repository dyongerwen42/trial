...
const TaskCreationForm = () => {
  ...
  const handleAddTask = useCallback(() => {
    ...
    saveData();
    resetForm();
  }, [proposedTasks, dispatch, saveData, selectedElements, selectedTasks]);
        ...
        return (
          <CardContent>
            ...
            {selectedElements.length > 0 && selectedTasks.length > 0 ? (
                  ...
                  return (
                    <Card key={`${element.spaceId}-${element.id}`} sx={{ p: 2 }}>
                      <Typography variant="subtitle1" gutterBottom>
                        {spaceName} > {element.name}
                      </Typography>
                      {selectedTasks.map((task, taskIndex) => {
                        const elementTaskKey = `${element.spaceId}-${element.id}-${taskIndex}`;
                        return (
                          <Grid container spacing={2} alignItems="center" key={taskIndex}>
                            <Grid item xs={12} md={8}>
                              <TextField
                                fullWidth
                                type="number"
                                label={`Geschatte prijs voor taak: ${
                                  task.adjustments.name || task.beschrijving
                                }`}
                                value={estimatedPrices[elementTaskKey] || ''}
                                onChange={(e) =>
              ...
              <Typography>Geen elementen of taken geselecteerd.</Typography>
            )}
          </CardContent>
        );
      case 4:
