...
export const  fetchMJOPData = async (
  ...
  try {
    const response = await axios.get(`http://34.34.100.96:5000/mjop/${id}`, { withCredentials: true });
...
export const saveMJOPData = async (id, formData, config, navigate) => {
  try {
    ...
    if (id) {
      ...
      await axios.put(`http://34.34.100.96:5000/mjop/${id}`, payload, config);
    } else {
      // Create new MJOP entry
      console.log(`[${new Date().toISOString()}] Creating new MJOP`);
      const response = await axios.post('http://34.34.100.96:5000/save-mjop', payload, config);
