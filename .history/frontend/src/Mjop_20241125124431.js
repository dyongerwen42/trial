...
const FullTaskManager = () => {
  ...
  const addTasksToElements = () => {
    ...
    const { name, cost, assignPricesIndividually, individualCosts } = taskProperties;

    if (!name) {
      setSnackbarMessage('Please enter a task group name!');
      setSnackbarOpen(true);
      return;
    }
    ...
    const newTaskGroup = {
      ...
      elements: selectedElementIds.map((elementId) => {
              ...
              <Accordion key={group.id} sx={{ mb: 2 }}>
                ...
                >
                  ...
                  <Typography variant="subtitle1" sx={{ marginLeft: 'auto', fontWeight: 600 }}>
                    €{group.assignPricesIndividually ? 'Varies' : group.cost}
                  </Typography>
                </AccordionSummary>
                <AccordionDetails>
      ...
      <Dialog open={isAddDialogOpen} onClose={() => setIsAddDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          ...
          <FormControlLabel
            ...
            label="Assign prices to each element individually"
          />
      ...
      <Dialog open={infoDialogOpen} onClose={handleCloseInfoDialog} maxWidth="md" fullWidth>
        ...
        <DialogContent>
          {currentElementInfo ? (
            ...
            <Typography>No element data available.</Typography>
          )}
        </DialogContent>
        <DialogActions>
