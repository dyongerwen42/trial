...
// Definieer de ernstcategorieën
const ernstCategorieën = ["ernstig", "serieus", "gering"];

// Functie om gebreken te extraheren en te combineren met gebreken uit de staat
const extractGebreken = (
  elementName,
  type,
  material,
  data,
  gebrekenFromState // Gebreken vanuit de huidige staat (inclusief aangepaste gebreken)
) => {
...
const GlobalElements = ({ t }) => {
  ...
  const handleSelectElement = useCallback(
    (element, isEditMode = false, fromTable = false) => {
      ...
      if (isEditMode && element.gebreken) {
        // Wanneer bewerken, behoud bestaande gebreken
        initialGebreken = element.gebreken;
      } else {
        // Wanneer toevoegen, initialise gebreken als leeg (geen geselecteerd)
        initialGebreken = {
          [category]: {
            [material]: [],
          },
      ...
      setMaterials(Object.keys(availableGebreken));
    ...
    [globalSpaces, setErrors, setNewElement, newElement.gebreken, errors]
  );
  ...
  const handleTypeChange = useCallback(
    (e) => {
      ...
      // Initialiseer gebreken correct
      const initialGebreken = {
        [selectedType]: {
    ...
    [newElement.name, inspectionTasks, errors, setErrors, setNewElement]
  );
  ...
  const handleMaterialChange = useCallback(
    (e) => {
      ...
      // Remove gebreken for the previous material if necessary
      ...
      if (
        ...
        // Verwijder gebreken voor de oude materiaalcategorie
        delete updatedGebreken[newElement.type][newElement.material];

        // Verwijder type als er geen materialen meer zijn
        if (
    ...
    [newElement.type, newElement.material, newElement.gebreken]
  );
  ...
  const handleAddCustomGebrek = useCallback(() => {
    ...
    const category = newElement.type; // Voeg toe onder huidige type
    const material = newElement.material; // Voeg toe onder huidige materiaal
    ...
    addGebrek(category, material, severity, gebrekName); // Correcte parameter volgorde

    setCustomGebrek("");
    setCustomGebrekSeverity("gering");
  }, [
    customGebrek,
    customGebrekSeverity,
    addGebrek,
    newElement.type,
    newElement.material,
    errors,
  ...
  // Handle removing a custom gebrek (optionally, if needed)
  const handleRemoveCustomGebrek = useCallback(
    (category, material, severity, gebrekName) => {
      removeGebrek(category, material, severity, gebrekName);
    },
    [removeGebrek]
  );

  // Helper function to render gebreken (met checkboxes)
  const renderGebreken = useCallback(() => {
    ...
    return (
      <Box>
        {ernstCategorieën.map((severity) => (
          <Box key={`severity-${severity}`} mb={2}>
            <Typography
              variant="body1"
              sx={{
                fontWeight: "bold",
            ...
            {availableGebreken[severity] && availableGebreken[severity].length > 0 ? (
              <List>
                {availableGebreken[severity].map((gebrek, index) => {
                  const isChecked =
                  ...
                  return (
                    <ListItem key={`gebrek-${severity}-${gebrek}-${index}`}>
                      <FormControlLabel
                        control={
                          <Checkbox
                            checked={isChecked}
  ...
  }, [newElement, addGebrek, removeGebrek, theme.palette, errors]);
  ...
  const handleSaveData = useCallback(async () => {
    try {
      setIsSaving(true); // Start loading indicator
      await saveData(); // Roep de saveData functie aan
  ...
  const handleTypeChangeFromSidePanel = useCallback(
    (element, selectedType) => {
      ...
      if (!gebrekenForType) {
        ...
        setErrors({
          ...errors,
          gebreken: `Geen gebreken gevonden voor type: ${selectedType}`,
        });
        return;
      }
      ...
      setNewElement({
        ...
        gebreken: {
          [selectedType]: {
            // Initialize gebreken per severity categorie
            [availableMaterials[0]]: [],
          },
        }, // Initialize gebreken met categorieën
        inspectionReport: {
          id: uuidv4(),
          description: "",
          inspectionDone: false,
      ...
      // Reset errors en stappen
      setErrors({});
      setFoutieveStappen([]);
      setActiveStep(0);
    },
    [errors, setErrors, setNewElement]
  );
                    ...
                    {newElement.inspectionReport.tasks.map((task, index) => (
                      <ListItem key={`review-task-${task.id}-${index}`}>
                        <ListItemText
                          ...
                          secondary={`Planned: ${task.planned ? "Ja" : "Nee"}`}
                        />
                      </ListItem>
                    ))}
                  </List>
