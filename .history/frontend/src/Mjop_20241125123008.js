...
const FullTaskManager = () => {
  ...
  const addTasksToElements = () => {
    ...
    const updatedGlobalElements = globalElements.map((element) => {
      if (selectedElementIds.includes(element.id)) {
        const newTask = {
          id: uuidv4(),
          name: currentCategory, // Gebruik de categorienaam als taaknaam
  ...
  const tasksByYearAndCategory = useMemo(() => {
    ...
    tasks.forEach((task) => {
      const { year, name } = task; // Gebruik 'name' nu als categorienaam
      if (!year) return; // Sla taken zonder jaar over

      const category = name; // Aangezien de taaknaam nu de categorienaam is
      if (!mapping[year]) {
        mapping[year] = {};
      }
                    ...
                    Object.entries(tasksByYearAndCategory[year]).map(([category, tasks]) => (
                      <Box key={category} sx={{ mb: 2, textAlign: 'left' }}>
                        ...
                        {tasks.map((task) => (
                          ...
                          >
                            {/* Weergave van categorienaam in plaats van taaknaam */}
                            <Typography variant="body2" sx={{ fontWeight: 500 }}>
                              {category}
                            </Typography>
