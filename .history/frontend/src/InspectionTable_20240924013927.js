...
const handleFakeRequestInspection = () => {
  alert('Inspectie is aangevraagd voor dit item');
};

const handleFakeRequestInspectionForSelected = (selectedIds) => {
  alert(`Inspectie aangevraagd voor geselecteerde items: ${selectedIds.join(', ')}`);
};
...
const InspectionTable = ({
    ...
    if (monthDiff <= 0) {
      return (
        <Tooltip title="Inspectie is achterstallig">
          <Error style={{ color: theme.palette.error.main, fontSize: '1.6rem' }} />
        </Tooltip>
      );
    } else if (monthDiff <= 1) {
      return (
        <Tooltip title="Inspectie binnenkort vereist">
          <Warning style={{ color: theme.palette.warning.main, fontSize: '1.6rem' }} />
        </Tooltip>
      );
    }
    return (
  ...
  <Button
    variant="contained"
    color="primary"
    onClick={() => handleFakeRequestInspectionForSelected(selectedRows)}
    sx={{
      ml: 2,
      borderRadius: 4,
      px: 3,
              ...
              >
                ...
                <TableCell sx={{ padding: '16px', fontSize: '1rem' }}>
                  <TextField
                    ...
                    InputProps={{
                      style: {
                        color: report.inspectionDate ? 'inherit' : theme.palette.text.disabled,
                        fontWeight: report.inspectionDate ? 'bold' : 'normal',
                        fontSize: report.inspectionDate ? '1.1rem' : '1rem',
                        backgroundColor: report.inspectionDate ? 'rgba(76, 175, 80, 0.1)' : 'inherit',
                        borderRadius: 2,
                      },
                    }}
                  />
                ...
                <TableCell sx={{ padding: '16px' }}>
                  <Box display="flex" gap={2}>
                    ...
                    <Tooltip title="Verzoek Inspectie">
                      <IconButton
                        onClick={handleFakeRequestInspection}
                        color="secondary"
                      >
                        <RequestPageIcon />
                      </IconButton>
