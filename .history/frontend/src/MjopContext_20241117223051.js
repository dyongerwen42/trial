...
export const MjopProvider = ({ children }) => {
  ...
  // **Aangepaste saveData functie die updatedGlobalElements accepteert**
// Define saveData as a regular async function
const saveData = async (updatedGlobalElements, skipValidation = false) => {
  alert('save');
  try {
    ...
    formData.append("globalElements", JSON.stringify(updatedGlobalElements)); // Gebruik de parameter
    ...
    console.log("Voorbereiden om data op te slaan met de volgende details:", {
      ...
      generalInfo: state.generalInfo,
      cashInfo: state.cashInfo,
      globalElements: updatedGlobalElements, // Gebruik de parameter
  ...
  // **Aangepaste handleAddElement zonder saveData aanroep**
  ...
  // **Aangepaste handleEditElement zonder saveData aanroep**
  ...
  // Corrected handleAddItemToPlanning
  ...
  const handleAddCustomItem = useCallback(
    (item) => {
      ...
      const generalElement = state.globalElements.find(
        (el) => el.name === "Algemeen" // Correcte naam
      );
      ...
      if (generalElement) {
        dispatch({
          ...
          payload: {
            globalElements: state.globalElements.map((element) =>
              element.name === "Algemeen" // Correcte naam
                ? {
                    ...element,
                    inspectionReport: [
                      {
  ...
  // **Define setErrors and setSuccessMessage**
  ...
  const contextValue = useMemo(
    () => ({
      state,
      dispatch,
      saveData, // Deze accepteert nu updatedGlobalElements als parameter
      calculateCurrentCash,
      getSaldoColor,
      setOfferGroups,
      setGlobalElements,
    ...
    [
      ...
      addGebrek,
      removeGebrek,
      setErrors,           // Added as dependency
      setSuccessMessage,  // Added as dependency
    ]
  );
