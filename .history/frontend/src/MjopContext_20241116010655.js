  ...
  const saveData = useCallback(
    async (updatedGlobalElements, skipValidation = false) => {
      ...
      try {
        ...
        // Handle propertyImage if it's a File object
        if (
        ...
        // Handle files in globalElements
        updatedGlobalElements.forEach((element, index) => {
          if (element.photos) {
            element.photos.forEach((photo, photoIndex) => {
              if (typeof photo !== "string") {
                formData.append(`elementPhotos_${element.id}_${photoIndex}`, photo);
              }
            });
          }
          if (element.documents) {
            element.documents.forEach((doc, docIndex) => {
              if (typeof doc !== "string") {
                formData.append(`elementDocuments_${element.id}_${docIndex}`, doc);
              }
            });
          }
        });

        // Handle files in globalSpaces
        state.globalSpaces.forEach((space, index) => {
          if (space.photos) {
            space.photos.forEach((photo, photoIndex) => {
              if (typeof photo !== "string") {
                formData.append(`spacePhotos_${space.id}_${photoIndex}`, photo);
              }
            });
          }
          if (space.documents) {
            space.documents.forEach((doc, docIndex) => {
              if (typeof doc !== "string") {
                formData.append(`spaceDocuments_${space.id}_${docIndex}`, doc);
              }
            });
          }
          if (
            ...
            !(space.image instanceof File)
          ) {
            formData.append(`spaceImage_${space.id}`, space.image);
          }
        });
