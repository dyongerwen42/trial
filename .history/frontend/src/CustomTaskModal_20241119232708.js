...
const InspectionModal = ({
    ...
    if (currentInspection) {
      ...
      const imagePreviews = (currentInspection.images || []).map((file) => {
        if (typeof file === 'string') {
          return `http://34.34.100.96:5000/${file}`;
  ...
  const handleAddImages = async (newImages) => {
    ...
    const updatedImagePreviews = [...imagePreviews, `http://34.34.100.96:5000/${data.filePath}`];
  ...
  const handleAddDocuments = async (newDocuments) => {
    ...
    const updatedDocumentPreviews = [...documentPreviews, `http://34.34.100.96:5000/${data.filePath}`];
                            ...
                            {task.documents.map((doc, docIndex) => {
                              ...
                              return (
                                <Box key={docIndex} position="relative" mr={2} mb={2}>
                                  ...
                                  <a
                                    href={`http://34.34.100.96:5000/${doc}`}
                                    download
                                    style={{ margin: '0 8px' }}
                                  >
                                    {icon}
                                  ...
                                  <Box display="flex" justifyContent="center" mt={1}>
                                    <IconButton
                                      ...
                                      onClick={() => {
                                        const link = document.createElement('a');
                                        link.href = `http://34.34.100.96:5000/${doc}`;
