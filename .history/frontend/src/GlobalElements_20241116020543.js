              ...
              >
                ...
                >
                  {materials.map((material, materialIndex) => (
                    <MenuItem key={`material-${material}-${materialIndex}`} value={material}>
                      {material}
                    </MenuItem>
                  ))}
                  <MenuItem key="material-Other" value="Other">Overig</MenuItem>
                </Select>
                {errors.material && (
              ...
              <FormControl fullWidth>
                ...
                <Box display="flex" flexWrap="wrap" mt={2}>
                  {imagePreviews.map((src, index) => (
                    <Box key={`image-${src}-${index}`} position="relative" mr={2} mb={2}>
                      <img
                        src={src}
                        alt={`Voorbeeld ${index}`}
                        style={{
              ...
              <FormControl fullWidth>
                ...
                <Box display="flex" flexWrap="wrap" mt={2}>
                  {documentPreviews.map((src, index) => {
                    ...
                    return (
                      <Box key={`document-${src}-${index}`} position="relative" mr={2} mb={2}>
            ...
            <tbody {...getTableBodyProps()}>
              {rows.map((row) => {
                ...
                return (
                  <tr
                    {...row.getRowProps()}
                    key={`table-row-${row.original.id || `row-${uuidv4()}`}`} // Ensure unique key
                    style={{
                  ...
                  >
                    {row.cells.map((cell) => (
                      <td
                        {...cell.getCellProps()}
                        key={`table-cell-${cell.column.id}-${uuidv4()}`} // Ensure unique key
                        style={{
                          padding: theme.spacing(1),
                          borderBottom: "1px solid #e0e0e0",
                        }}
        ...
        >
          ...
          <Box display="flex" flexWrap="wrap" mt={2}>
            {selectedPhotos.map((src, index) => (
              <Box key={`modal-photo-${src}-${index}`} position="relative" mr={2} mb={2}>
                <img
                  src={src}
                  alt={`Photo ${index}`}
                  style={{
        ...
        >
          ...
          <Box display="flex" flexWrap="wrap" mt={2}>
            {selectedDocuments.map((src, index) => {
              ...
              return (
                <Box key={`modal-document-${src}-${index}`} position="relative" mr={2} mb={2}>
                  <a href={src} download style={{ margin: "0 8px" }}>
                    {icon}
                  </a>
                  <Box display="flex" justifyContent="center" mt={1}>
