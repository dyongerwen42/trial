...
export const MjopProvider = ({ children }) => {
  ...
  const handleAddItemToPlanning = useCallback((task, elementId) => {
    ...
    dispatch({ type: 'ADD_TASK', payload: { elementId, tasks: [task] } });
  }, [dispatch, logAction]);
  ...
  const handleAddCustomItem = useCallback((item) => {
    ...
    if (generalElement) {
      dispatch({
        ...
        payload: { elementId: generalElement.id, item }
      });
    } else {
      const newElement = {
        id: uuidv4(),
      ...
      dispatch({ type: 'ADD_GLOBAL_ELEMENT', payload: newElement });
    }
  ...
  const contextValue = useMemo(() => ({
    ...
    handleDeleteItem,
    handleAddCustomItem,
    setNewSpace, // Make sure to include this if you're managing newSpace in state
  }), [
    state,
    dispatch,
    saveData,
