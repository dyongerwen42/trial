...
export const MjopProvider = ({ children }) => {
  ...
  const setNewSpace = useCallback((updatedFields) => {
  ...
  const saveData = useCallback(async () => {
    ...
    try {
      ...
      formData.append('globalSpaces', JSON.stringify(state.globalSpaces)); // Check that this includes the updated spaces
      ...
      // Log formData values to confirm content before sending to the backend
      console.log("Preparing to save data with the following details:", {
        mjopId,
        generalInfo: state.generalInfo,
        cashInfo: state.cashInfo,
        globalElements: state.globalElements,
        globalSpaces: state.globalSpaces, // Verify this includes the updated spaces
      ...
      dispatch({ type: 'SET_ERRORS', payload: { general: 'Error saving data. Please try again later.' } });
    }
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    // Log the state at the start of submission for validation
  ...
  const handleAddElement = useCallback(async (element) => {
    ...
    // Immediately call saveData to persist new elements to the backend
    await saveData();
  }, [dispatch, saveData, logAction]);
  ...
  const handleAddCustomItem = useCallback((item) => {
    ...
    const generalElement = state.globalElements.find((el) => el.name === 'Algemeneen');
