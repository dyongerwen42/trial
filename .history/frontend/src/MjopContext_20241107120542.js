...
export const MjopProvider = ({ children }) => {
  ...
  const saveData = useCallback(async () => {
    ...
    try {
      ...
      formData.append('globalSpaces', JSON.stringify(state.globalSpaces)); // Log to verify this data
      ...
      // Log the formData values for verification
      console.log("Saving data with the following details:", {
        mjopId,
        generalInfo: state.generalInfo,
        cashInfo: state.cashInfo,
        globalElements: state.globalElements,
        globalSpaces: state.globalSpaces, // Check that this includes the updated spaces
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      ...
      dispatch({ type: 'SET_ERRORS', payload: { general: 'Error during submission. Please try again.' } });
    } finally {
      dispatch({ type: 'SET_SAVING', payload: false });
    }
