...
const TaskGroupDialog = ({
  ...
  const handleInputChange = useCallback((field, value) => {
    const numericFields = ['cost', 'amount', 'totalSquareMeters', 'indexationRate', 'periodicityMonths', 'totalYears'];
    setTaskProperties((prev) => ({
      ...prev,
      [field]: numericFields.includes(field) ? (value === '' ? '' : parseFloat(value)) : value,
    }));
  }, [setTaskProperties]);
    ...
    (elementId, value) => {
      setTaskProperties((prev) => ({
        ...
        individualCosts: {
          ...
          [elementId]: value === '' ? '' : parseFloat(value),
        },
      }));
    },
    [setTaskProperties]
  ...
  // Snackbar state binnen de dialog
  const [snackbarMessage, setSnackbarMessage] = useState('');
  const [snackbarOpen, setSnackbarOpen] = useState(false);

  // Effect om te resetten wanneer de dialog sluit
  useEffect(() => {
    if (!open) {
      setTaskProperties({
        id: null,
  ...
  return (
    ...
    >
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                ...
                type="date"
                fullWidth
                value={taskProperties.groupDate ? format(new Date(taskProperties.groupDate), 'yyyy-MM-dd') : ''}
                onChange={(e) =>
                  handleInputChange('groupDate', e.target.value ? parseISO(e.target.value) : null)
                }
                InputLabelProps={{
                  shrink: true,
                }}
            ...
            {/* Bedrag (nu Tijd in Maanden) */}
            <Grid item xs={12} sm={6}>
              <TextField
                label="Tijd per Taakgroep (maanden)"
                type="number"
                fullWidth
            ...
            {taskProperties.periodic && (
              <>
                ...
                {/* Toevoegen van Total Years */}
                <Grid item xs={12} sm={6}>
                  <TextField
                    label="Totale Jaren"
                    type="number"
                    fullWidth
                    value={taskProperties.totalYears}
                    onChange={(e) => handleInputChange('totalYears', e.target.value)}
                    required
                    inputProps={{ min: 1, step: '1' }}
                    aria-label="Totale Jaren"
                    variant="outlined"
                  />
                </Grid>
                {/* Toevoegen van Indexation Rate */}
                {taskProperties.indexation && (
                  <Grid item xs={12} sm={6}>
                    <TextField
                      label="Indexatievoet (%) per jaar"
                      type="number"
                      fullWidth
                      ...
                      required
                      inputProps={{ min: 0, step: '0.1' }}
                      aria-label="Indexatievoet per jaar"
                      variant="outlined"
                    />
                  </Grid>
                )}
      ...
      {/* Snackbar voor feedback berichten binnen de dialog */}
      <Snackbar
        open={snackbarOpen}
...
TaskGroupDialog.propTypes = {
  ...
  taskProperties: PropTypes.shape({
    ...
    groupDate: PropTypes.oneOfType([PropTypes.string, PropTypes.instanceOf(Date)]).isRequired,
    cost: PropTypes.oneOfType([PropTypes.string, PropTypes.number]).isRequired,
    amount: PropTypes.oneOfType([PropTypes.string, PropTypes.number]).isRequired, // Tijd in maanden
    ...
    totalYears: PropTypes.oneOfType([PropTypes.string, PropTypes.number]),
