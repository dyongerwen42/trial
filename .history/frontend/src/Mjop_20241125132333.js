...
const FullTaskManager = () => {
  ...
  const saveEditedTaskGroup = () => {
    ...
    // Update logic for edited task group
    // ... [Hier zou je de logic toevoegen om de bestaande taakgroep te updaten]
  ...
  const tasksByYear = useMemo(() => {
    ...
    const tasksByYear = tasks.reduce((acc, task) => {
      const year = task.endDate ? new Date(task.endDate).getFullYear() : null;
      if (year) {
        if (!acc[year]) {
          acc[year] = [];
        }
              ...
              <Accordion key={group.id} sx={{ mb: 2 }}>
                ...
                >
                  <Typography variant="subtitle1" sx={{ fontWeight: 600 }}>
                    {group.name} (Datum: {new Date(group.groupDate).getFullYear()})
                  </Typography>
                  {/* Cost Display */}
