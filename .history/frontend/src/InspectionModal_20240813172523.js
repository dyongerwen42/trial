...
const InspectionModal = ({
  ...
  useEffect(() => {
    if (currentInspection) {
      ...
      setInspectionData({
        ...
        overallConditionScore: currentInspection.overallConditionScore || 1, // Gebruik de correcte property hier
      });
  ...
  const handleRemoveMistake = (index) => {
    const updatedMistakes = inspectionData.mistakes.filter(
      (_, i) => i !== index
    );
    updateInspectionField("mistakes", updatedMistakes);
    calculateOverallConditionScore(updatedMistakes);
  ...
  const validateForm = () => {
    ...
    if (
      inspectionData.mistakes.some(
        (m) => !m.severity || !m.omvang || !m.description
      )
    ) {
      errors.mistakes = "Vul alle velden in voor gebreken.";
    }

    if (
      tasks.some(
        (t) => !t.name || !t.description || !t.estimatedPrice || !t.ultimateDate
      )
    ) {
  ...
  const tabLabelWithError = (label, errorKey) => (
    <Badge variant="dot" color="error" invisible={!validationErrors[errorKey]}>
      {label}
    </Badge>
  );
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} sm={6}>
                <TextField
                  ...
                  onChange={(e) =>
                    updateInspectionField("condition", e.target.value)
                  }
                  variant="outlined"
                >
              ...
              <Box display="flex" flexDirection="row" height="100%">
                ...
                >
                  {inspectionData.mistakes.map((mistake, index) => (
                    ...
                    >
                      {/* Conditionally render the Gebrek Categorie field */}
