          ...
          ? action.payload.globalElements.map((element) => ({
              ...
              inspectionReport: Array.isArray(element.inspectionReport)
                ? element.inspectionReport.map(report => ({
                    ...report,
                    overallConditionScore: report.overallConditionScore || 1, // Zorg ervoor dat de score aanwezig is
                  }))
                : [
                    {
                      id: uuidv4(),
    ...
    case ADD_GLOBAL_ELEMENT:
      ...
      return {
        ...
        globalElements: [
          ...
          {
            ...action.payload,
            gebreken: action.payload.gebreken || { // Zorg ervoor dat gebreken vanuit payload gebruikt worden
              ernstig: [],
              serieus: [],
              gering: [],
            },
      ...
      return {
        ...
        globalElements: state.globalElements.map((el) =>
          el.id === action.payload.id
            ? {
                ...el, // Zorg ervoor dat bestaande data behouden blijft
                ...action.payload,
                gebreken: action.payload.gebreken || el.gebreken, // Gebruik bestaande gebreken als er geen nieuwe zijn
                inspectionReport: Array.isArray(action.payload.inspectionReport)
                  ? action.payload.inspectionReport.map(report => ({
                      ...report,
                      overallConditionScore: report.overallConditionScore || el.inspectionReport.find(r => r.id === report.id)?.overallConditionScore || 1,
                    }))
                  : el.inspectionReport,
...
export const MjopProvider = ({ children }) => {
  ...
  const initializeData = useCallback(
    ...
    [dispatch] // Removed state.globalElements from dependencies to prevent unnecessary re-renders
  );
