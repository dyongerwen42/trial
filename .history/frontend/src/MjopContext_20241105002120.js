...
export const MjopProvider = ({ children }) => {
  ...
  const initializeData = useCallback(async (id) => {
    try {
        console.log("Initializing Data for ID:", id);

        // Combine all data in one payload to reduce multiple dispatch calls
        const data = await fetchMJOPData(id);

        // Log data to confirm the correct structure
        console.log("Fetched data:", data);

        // Ensure all required fields are set or fallback to default state structure
        const payload = {
            ...
            globalElements: data.globalElements || initialState.globalElements,
            globalSpaces: data.globalSpaces || initialState.globalSpaces,
            globalDocuments: data.globalDocuments || initialState.globalDocuments,
            offerGroups: data.offerGroups || initialState.offerGroups,
        };
