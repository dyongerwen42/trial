...
const extractGebreken = (elementName, type, material, data, customGebreken) => {
  ...
  if (customGebreken && customGebreken[type] && customGebreken[type][material]) {
    for (const severity in customGebreken[type][material]) {
      ...
      gebreken[severity] = [...new Set([...gebreken[severity], ...customGebreken[type][material][severity]])];
    }
  }
...
const GlobalElements = ({ t }) => {
  const {
    state: { globalElements, globalSpaces, newElement, errors, customGebreken }, // Voeg customGebreken toe
    handleAddElement,
    handleEditElement,
    saveData,
    handleDeleteElement,
    ...
    addGebrek,
    removeGebrek,
    addCustomGebrek, // Zorg dat deze functie aanwezig is in de context
  } = useMjopContext();
