...
const TaskCreationForm = () => {
  ...
  const handleAddTask = useCallback(() => {
    ...
    selectedElements.forEach((element) => {
      ...
      tasksToAdd.forEach((task, taskIndex) => {
        let taskName = task.adjustments?.name || task.name || task.beschrijving;
        let taskDescription =
          task.adjustments?.description || task.description || task.toelichting;
        let taskUrgency = task.adjustments?.urgency || task.urgency || 3; // Standaard urgentie
        ...
        let newStartDate = task.adjustments?.startDate || new Date();
        let durationDays = task.adjustments?.durationDays
          ? parseInt(task.adjustments.durationDays)
          : 0;

        let newUltimateDate = new Date(task.adjustments?.ultimateDate || newStartDate);
        ...
        if (task.adjustments?.useInterval) {
          newTask.planned.useInterval = true;
          newTask.planned.intervalYears = parseFloat(task.adjustments.intervalYears);
          newTask.planned.totalYears = parseFloat(task.adjustments.totalYears);
          newTask.planned.inflationRate = parseFloat(task.adjustments.inflationRate);
        }
    ...
    saveData();
    resetForm();
  }, [customTask, selectedElements, selectedTasks, estimatedPrices, dispatch, saveData]);
  ...
  const Step2FillTaskDetails = () => {
    ...
    const handleAddCustomTask = () => {
      ...
      setSelectedTasks((prevTasks) => [...prevTasks, { ...newTask, adjustments: {} }]);
      setNewTask({
        name: '',
        description: '',
        urgency: 3,
            ...
            <Accordion key={index} defaultExpanded>
              <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                <Typography variant="subtitle1">
                  {task.adjustments.name || task.beschrijving || task.name}
                </Typography>
              </AccordionSummary>
              <AccordionDetails>
                <Grid container spacing={2}>
                  <Grid item xs={12} md={6}>
                    <TextField
                      fullWidth
                      label="Naam"
                      value={task.adjustments.name || task.beschrijving || task.name}
                    ...
                    <TextField
                      ...
                      onChange={(e) =>
                        handleTaskAdjustment(index, 'estimatedPrice', e.target.value)
                      }
                      variant="outlined"
                      size="medium"
                      type="number"
                    ...
                    <DatePicker
                      selected={task.adjustments.startDate || new Date()}
                      onChange={(date) => handleTaskAdjustment(index, 'startDate', date)}
                    ...
                    <TextField
                      ...
                      type="number"
                      label="Duur in dagen"
                      value={task.adjustments.durationDays || ''}
                      onChange={(e) => handleTaskAdjustment(index, 'durationDays', e.target.value)}
                      variant="outlined"
                      size="medium"
                    />
                  </Grid>
                  ...
                  {task.adjustments.setDirectToWorkDate && (
                    <Box sx={{ pl: 4, mt: 1 }}>
                      ...
                      <DatePicker
                        ...
                        onChange={(date) => handleTaskAdjustment(index, 'workDate', date)}
                        dateFormat="dd/MM/yyyy"
                ...
                <Box sx={{ mt: 2 }}>
                  ...
                  <DatePicker
                    selected={
                      ...
                      addDays(
                        task.adjustments.startDate || new Date(),
                        task.adjustments.durationDays
                          ? parseInt(task.adjustments.durationDays)
                          : 0
                      )
                    }
                    onChange={(date) => handleTaskAdjustment(index, 'endDate', date)}
                    dateFormat="dd/MM/yyyy"
                  ...
                  selectedElements[0].type.toLowerCase().includes('climate') && (
                    <Box sx={{ mt: 2 }}>
                      ...
                      <TextField
                        ...
                        onChange={(e) =>
                          handleTaskAdjustment(index, 'comment', e.target.value)
                        }
                        variant="outlined"
                        size="medium"
                        multiline
                ...
                <TableBody>
                  {selectedElements.map((element) => {
                    ...
                    return selectedTasks.map((task, taskIndex) => {
                      ...
                      let newUltimateDate = new Date(
                        task.adjustments?.ultimateDate || new Date()
                      );
                      let newStartDate = task.adjustments?.startDate || new Date();
                      let newWorkDate = null;
                      let durationDays = task.adjustments?.durationDays
                      ...
                      return (
                        <TableRow key={`${element.id}-${taskIndex}`}>
                          ...
                          <TableCell>{task.adjustments?.comment || '-'}</TableCell>
                          <TableCell>
                            {task.adjustments?.contractCost
                              ? `€ ${parseFloat(task.adjustments.contractCost).toFixed(
                                  2
                                )}`
                              : '-'}
                          </TableCell>
                          <TableCell>
                            {task.adjustments?.contractDuration
                              ? `${task.adjustments.contractDuration} jaar(s)`
                              : '-'}
                          </TableCell>
                        </TableRow>
                      );
