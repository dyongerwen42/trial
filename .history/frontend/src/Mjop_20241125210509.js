...
const FullTaskManager = () => {
  ...
  return (
    <Box sx={{ height: '100vh', backgroundColor: '#f0f2f5', overflow: 'hidden' }}>
      <Grid container sx={{ height: '100%' }}>
                ...
                >
                  <AccordionSummary
                    ...
                    sx={{
                      ...
                      '&.Mui-expanded': { backgroundColor: '#f1f1f1' },
                    }}
                  >
                    <FormControlLabel
                      control={
                              ...
                              <CardHeader
                                avatar={
                                  <Checkbox
                                    ...
                                    onChange={() =>
                                      toggleElementSelection(category, element.id)
                                    }
                                    color="primary"
                                  />
                                }
        ...
        {/* Main Panel */}
        <Grid item xs={12} md={9} sx={{ overflowY: 'auto' }}>
          <Box sx={{ p: 2 }}>
                      ...
                      {taskGroupsByYear[year] && taskGroupsByYear[year].length > 0 ? (
                        taskGroupsByYear[year].map((group) => (
                          <Card
                            ...
                            onClick={() => handleOpenTaskGroupInfoDialog(group)}
                          >
                            <CardHeader
                              avatar={
        ...
        <DialogContent dividers>
          {currentElementInfo ? (
            <Box>
              ...
              {/* Documents */}
              {currentElementInfo.documents &&
                currentElementInfo.documents.length > 0 && (
                  <Box sx={{ mt: 2 }}>
                    <Typography variant="subtitle1" gutterBottom>
                      Documenten:
                    </Typography>
              ...
              {/* Defects */}
              {currentElementInfo.gebreken &&
                Object.keys(currentElementInfo.gebreken).length > 0 && (
                  <Box sx={{ mt: 2 }}>
                    <Typography variant="subtitle1" gutterBottom>
                      Gebreken:
                    </Typography>
                    {Object.entries(currentElementInfo.gebreken).map(
                      ([severity, issues]) => (
              ...
              {/* Inspection Reports */}
              {currentElementInfo.inspectionReport &&
                currentElementInfo.inspectionReport.length > 0 && (
                  <Box sx={{ mt: 2 }}>
                    <Typography variant="subtitle1" gutterBottom>
                      Inspectierapporten:
                    </Typography>
                    ...
                    <Card key={task.id} variant="outlined" sx={{ mb: 2 }}>
                      ...
                      <CardContent>
                        ...
                        <Typography variant="body2" gutterBottom>
                          ...
                          {task.estimatedPrice || 'Onbekend'}
                        </Typography>
                        <Typography variant="body2" gutterBottom>
                          <strong>Ultimatum Datum:</strong>{' '}
                          {task.ultimateDate
              ...
              {/* Annotations */}
              {currentElementInfo.annotations &&
                currentElementInfo.annotations.length > 0 && (
                  <Box sx={{ mt: 2 }}>
                    <Typography variant="subtitle1" gutterBottom>
                      Annotaties:
                    </Typography>
                    {currentElementInfo.annotations.map((annotation, idx) => (
                      <Box key={idx} sx={{ mb: 1 }}>
                        ...
                        <Typography variant="body2">
                          ...
                          width: {annotation.width.toFixed(3)}, height:{' '}
