...
const GlobalElements = ({ t }) => {
  ...
  const handleAddCustomGebrek = () => {
    ...
    // Voeg gebrek toe onder de huidige type en material
    const category = newElement.type; // 'category' verwijst naar 'severity' in MjopContext.js
    if (!category) {
      console.error("Type (category) is niet geselecteerd.");
      return;
    }

    addGebrek(severity, gebrekName); // Corrected parameter order
        ...
        {Object.keys(availableGebreken).map((severity, severityIndex) => (
          <Box key={`severity-${severity}-${severityIndex}`} mb={2}>
            <Box display="flex" alignItems="center" justifyContent="space-between">
              ...
              <Button
                size="small"
                onClick={() => handleSelectAllGebreken(severity, availableGebreken[severity])}
              >
                Selecteer Alles
              </Button>
            </Box>
            {availableGebreken[severity].length > 0 ? (
              <List>
                {availableGebreken[severity].map((gebrek, gebrekIndex) => {
                  const isChecked =
                    newElement.gebreken[severity] &&
                    newElement.gebreken[severity].includes(gebrek);
