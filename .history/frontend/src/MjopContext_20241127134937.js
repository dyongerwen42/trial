...
// **New Action Type for Setting Data Loaded**
...
const initialState = {
  ...
  isSaving: false,
  isDataLoaded: false, // Reintroduced isDataLoaded in state
  errors: {},
  success: null,
  tabErrors: {
    generalInfo: false,
...
export const MjopProvider = ({ children }) => {
  ...
  const saveData = useCallback(
    async (updatedGlobalElements = state.globalElements, skipValidation = false) => {
      try {
        ...
        // Optionally perform validation here if not skipped
  ...
  const addOfferGroupFunc = useCallback(
    (offerGroup) => {
  ...
  const handleItemChange = useCallback(
    (taskId, field, value, elementId) => {
      logAction("Changing Item", { taskId, field, value, elementId });

      // Validate that elementId is provided
      if (!elementId) {
        console.error("Element ID is required to edit a task.");
        return;
      }
  ...
  const handleDeleteItem = useCallback(
    (taskId, elementId) => {
      ...
      // Validate that elementId is provided
      if (!elementId) {
        console.error("Element ID is required to delete a task.");
        return;
      }
  ...
  const contextValue = useMemo(
    () => ({
      state,
      dispatch,
      isDataLoaded: state.isDataLoaded, // Included isDataLoaded in context
      ...
      // **OfferGroup Functions**
      addOfferGroup: addOfferGroupFunc,      // Exposed directly
      editOfferGroup: editOfferGroupFunc,     // Exposed directly
      deleteOfferGroup: deleteOfferGroupFunc,   // Exposed directly
    }),
    [
      state,
      saveData,
