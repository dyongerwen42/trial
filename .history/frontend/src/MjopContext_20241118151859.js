...
export const MjopProvider = ({ children }) => {
  ...
  // Effect om saveData aan te roepen wanneer globalElements verandert en isSaving is true
  useEffect(() => {
    if (state.isSaving) {
      saveData();
    }
  }, [state.globalElements, state.isSaving, saveData]);
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      // setIsSaving(true) is al gedaan via dispatch
      await saveData();
      dispatch({ type: "SET_SAVING", payload: false });
