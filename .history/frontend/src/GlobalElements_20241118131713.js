...
// Helper functie om gebreken op te halen
...
const GlobalElements = ({ t }) => {
  ...
  const handleSaveElement = async () => {
    ...
    // Validatie voor levensduur, aanschafDatum, vervangingsKosten
    if (!newElement.levensduur) newErrors.levensduur = "Levensduur is verplicht";
    if (!newElement.aanschafDatum) newErrors.aanschafDatum = "Aanschaf Datum is verplicht";
    if (!newElement.vervangingsKosten) newErrors.vervangingsKosten = "Vervangings Kosten zijn verplicht";

    // Validatie voor gebreken: minimaal één ernstig en serieus gebrek
    if (
      !newElement.gebreken ||
      !newElement.gebreken.ernstig ||
      newElement.gebreken.ernstig.length === 0
    ) {
      newErrors.gebreken = "Minimaal één ernstig gebrek is vereist";
    }
    if (
      !newElement.gebreken ||
      !newElement.gebreken.serieus ||
      newElement.gebreken.serieus.length === 0
    ) {
      newErrors.gebreken = "Minimaal één serieus gebrek is vereist";
    }

    if (Object.keys(newErrors).length > 0) {
      setErrors(newErrors);
      const invalidSteps = [];
      if (newErrors.name || newErrors.interval || newErrors.description || newErrors.categories) invalidSteps.push(0);
      if (newErrors.spaceId) invalidSteps.push(1);
      if (newErrors.photos || newErrors.documents) invalidSteps.push(2);
      if (newErrors.gebreken) invalidSteps.push(3);
