...
const FullTaskManager = () => {
  ...
  return (
    <Box sx={{ position: 'relative', height: '100vh', backgroundColor: '#e0e0e0' }}>
              ...
              <Accordion key={category} sx={{ mb: 1, boxShadow: 'none' }}>
                ...
                <AccordionDetails>
                  {elements.map((element) => (
                    <Box key={element.id} sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                      <Checkbox
                        checked={
      ...
      <Dialog open={isAddDialogOpen} onClose={() => setIsAddDialogOpen(false)}>
        ...
        <DialogActions>
          <Button onClick={() => setIsAddDialogOpen(false)} color="secondary" sx={{ textTransform: 'none' }}>
            Cancel
          </Button>
          <Button
