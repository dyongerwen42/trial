...
export const MjopProvider = ({ children }) => {
  ...
  const setGeneralInfo = (updatedFields) => dispatch({ type: 'SET_GENERAL_INFO', payload: updatedFields });
  const setOfferGroups = (offerGroups) => dispatch({ type: 'SET_DATA', payload: { offerGroups } });
  const setGlobalElements = (globalElements) => dispatch({ type: 'SET_DATA', payload: { globalElements } });
  const setGlobalSpaces = (globalSpaces) => dispatch({ type: 'SET_DATA', payload: { globalSpaces } });
    ...
    const initializeData = async () => {
      try {
        ...
        if (data) {
          ...
          console.warn('No data returned for MJOP ID:', id);
        }
      } catch (err) {
  ...
  const saveData = async () => {
    if (!handleValidation()) {
      dispatch({ type: 'SET_ERRORS', payload: { general: 'Validation errors need to be resolved.' } });
      return;
    }
    ...
    try {
      ...
      dispatch({ type: 'SET_ERRORS', payload: { general: 'An error occurred while saving data.' } });
    }
  };
