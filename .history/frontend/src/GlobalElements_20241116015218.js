...
const GlobalElements = ({ t }) => {
  ...
  const handleFileChange = (e) => {
    ...
    setNewElement({
      ...newElement,
      photos: [...(newElement.photos || []), ...files],
    });
  };
  ...
  const handleDocumentChange = (e) => {
    ...
    setNewElement({
      ...newElement,
      documents: [...(newElement.documents || []), ...files],
    });
  };
  ...
  const handleSaveElement = useCallback(async () => {
    ...
    try {
      setIsSaving(true); // Start the loading indicator
      ...
      if (isEditing) {
        ...
        // Compute the updated globalElements array
        updatedGlobalElements = globalElements.map((el) =>
          el.id === newElement.id ? newElement : el
        );
      } else {
        const newElementWithId = handleAddElement(newElement); // Add the new element and receive it with ID
      ...
      await saveData(updatedGlobalElements); // Pass the updated array
      ...
      setIsSaving(false); // Stop the loading indicator
    }
  }, [
    isEditing,
    newElement,
  ...
  const handleAddAnnotation = (position) => {
    const newAnnotation = {
      ...position,
      id: uuidv4(), // Ensure each annotation has a unique ID
    };
    setNewElement({
      ...newElement,
        ...
        {Object.keys(availableGebreken).map((severity) => (
          <Box key={`${severity}-${uuidv4()}`} mb={2}>
            ...
            {availableGebreken[severity].length > 0 ? (
              <List>
                {availableGebreken[severity].map((gebrek, idx) => (
                  <ListItem
                    key={`${severity}-${gebrek}-${idx}`} // Ensure uniqueness
                    secondaryAction={
                      <IconButton
                        edge="end"
                        aria-label="delete"
              ...
              <FormControl fullWidth>
                ...
                <Box display="flex" flexWrap="wrap" mt={2}>
                  {imagePreviews.map((src, index) => (
                    <Box key={`image-${index}-${uuidv4()}`} position="relative" mr={2} mb={2}>
                      <img
                        src={src}
                        alt={`Voorbeeld ${index}`}
                        style={{
              ...
              <FormControl fullWidth>
                ...
                <Box display="flex" flexWrap="wrap" mt={2}>
                  {documentPreviews.map((src, index) => {
                    ...
                    return (
                      <Box key={`document-${index}-${uuidv4()}`} position="relative" mr={2} mb={2}>
        ...
        >
          ...
          <List>
            {filteredElements.map((element, index) => {
              ...
              return (
                <React.Fragment key={element.id || `element-${index}-${uuidv4()}`}>
                  <ListItem
                    button
                  ...
                  {isExpanded && (
                    ...
                    >
                      {Object.keys(
                        ...
                        <ListItem
                          button
                          key={`${element.id}-${type}-${uuidv4()}`} // Ensure uniqueness
                          onClick={() =>
                ...
                <tr {...headerGroup.getHeaderGroupProps()} key={headerGroup.id}>
                  {headerGroup.headers.map((column) => (
                    <th
                      ...
                      key={`${column.id}-${uuidv4()}`} // Ensure uniqueness
                      style={{
            ...
            <tbody {...getTableBodyProps()}>
              {rows.map((row) => {
                ...
                return (
                  <tr
                    {...row.getRowProps()}
                    key={`row-${row.id}-${uuidv4()}`} // Ensure uniqueness
                    style={{
                  ...
                  >
                    {row.cells.map((cell) => (
                      <td
                        {...cell.getCellProps()}
                        key={`${cell.column.id}-${cell.row.id}-${uuidv4()}`} // Ensure uniqueness
                        style={{
                          padding: theme.spacing(1),
                          borderBottom: "1px solid #e0e0e0",
                        }}
        ...
        >
          ...
          <Box display="flex" flexWrap="wrap" mt={2}>
            {selectedPhotos.map((src, index) => (
              <Box key={`modal-photo-${index}-${uuidv4()}`} position="relative" mr={2} mb={2}>
                <img
                  src={src}
                  alt={`Photo ${index}`}
                  style={{
        ...
        >
          ...
          <Box display="flex" flexWrap="wrap" mt={2}>
            {selectedDocuments.map((src, index) => {
              ...
              return (
                <Box key={`modal-document-${index}-${uuidv4()}`} position="relative" mr={2} mb={2}>
                  <a href={src} download style={{ margin: "0 8px" }}>
                    {icon}
                  </a>
                  <Box display="flex" justifyContent="center" mt={1}>
