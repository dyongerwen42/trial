...
const Planning = () => {
  ...
  const {
    ...
    cashInfo = { currentCash: 0, monthlyContribution: 0, reserveDate: new Date() }, // Default object with safe values
    offerGroups = [], // Default to an empty array if undefined
    setGlobalElements = () => {}, // Default to a no-op function
    setOfferGroups = () => {}, // Default to a no-op function
  } = useMjopContext();
