...
const GlobalElements = ({ t }) => {
  ...
  const handleMaterialChange = (e) => {
    ...
    if (newElement.type && newElement.gebreken[newElement.type]) {
      // Itereer over alle ernst categorieën en verwijder gebreken voor de oude materiaal
      Object.keys(newElement.gebreken[newElement.type]).forEach((severity) => {
        if (updatedGebreken[newElement.type][severity][selectedMaterial]) {
          delete updatedGebreken[newElement.type][severity][selectedMaterial];
        }
      });
    }
  ...
  const handleSelectGebrek = (category, severity, material, gebrekName) => {
    ...
    const isSelected =
      ...
      newElement.gebreken[category][severity] &&
      newElement.gebreken[category][severity][material] &&
      newElement.gebreken[category][severity][material].includes(gebrekName);
  ...
  const handleChangeSeverity = (category, severity, material, gebrekName, newSeverity) => {
  ...
  const renderGebreken = () => {
    ...
    return Object.keys(availableGebreken).map((severity) => (
      <Box key={severity} mb={2}>
        ...
        {Object.keys(availableGebreken[severity]).map((materialKey) => (
          <Box key={materialKey} ml={2} mb={1}>
            <Typography variant="subtitle2" sx={{ textTransform: 'capitalize' }}>
              Materiaal: {materialKey}
            </Typography>
            <Grid container spacing={1}>
              {availableGebreken[severity][materialKey].map((gebrek) => (
                      ...
                      control={
                        <Checkbox
                          checked={
                            newElement.gebreken[category] &&
                            newElement.gebreken[category][severity] &&
                            newElement.gebreken[category][severity][materialKey] &&
                            newElement.gebreken[category][severity][materialKey].includes(gebrek)
                          }
                          onChange={() =>
                            handleSelectGebrek(category, severity, materialKey, gebrek)
                          }
                        />
                      }
                      label={gebrek}
