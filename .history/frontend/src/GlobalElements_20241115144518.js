...
const GlobalElements = ({ t }) => {
  ...
  const [documentsModalOpen, setDocumentsModalOpen] = useState(false); // Zorg dat dit is gedefinieerd
  ...
  // Definieer de ontbrekende functie om de documenten modaal te sluiten
  ...
  // **Definieer de ontbrekende functies**

  /**
   * Functie om gebreken te selecteren/deselecteren
   */
  const handleSelectGebrek = (category, severity, material, gebrekName) => {
    ...
    if (isSelected) {
      // Verwijder het gebrek via context
      removeGebrek(category, material, severity, gebrekName);
    } else {
      // Voeg het gebrek toe via context
      addGebrek(category, material, severity, gebrekName);
    }
  };

  /**
   * Functie om een aangepaste gebrek toe te voegen
   */
  ...
  return (
    <Box display="flex" height="100vh">
      ...
      <Modal
        ...
        onClose={handleCloseDocumentsModal} // Zorg dat deze functie is gedefinieerd
