...
const InvoiceDashboard = () => {
  ...
  const kpis = useMemo(() => {
    ...
    const totalOutstanding = formattedInvoices
      .filter((inv) => inv.paymentStatus === 'niet betaald')
      .reduce((sum, inv) => sum + (inv.price || 0), 0);
    
    const totalPaid = formattedInvoices
      .filter((inv) => inv.paymentStatus === 'betaald')
      .reduce((sum, inv) => sum + (inv.price || 0), 0);
    
    const overdueCount = formattedInvoices.filter(
      (inv) => inv.paymentStatus === 'niet betaald' && new Date(inv.dueDate) < new Date()
    ).length;

    const totalValue = totalOutstanding + totalPaid;
    const averageInvoiceValue = totalInvoices > 0 ? totalValue / totalInvoices : 0;

    console.log("KPI Calculation Debugging", {
      totalInvoices,
      totalOutstanding,
      totalPaid,
      overdueCount,
