...
const InvoiceDashboard = () => {
  ...
  const kpis = useMemo(() => {
    ...
    const totalOutstanding = formattedInvoices.reduce((sum, inv) => (inv.paymentStatus === 'niet betaald' ? sum + inv.price : sum), 0);
    const totalPaid = formattedInvoices.reduce((sum, inv) => (inv.paymentStatus === 'betaald' ? sum + inv.price : sum), 0);
    ...
    return {
      totalInvoices,
      totalOutstanding: totalOutstanding ?? 0,  // Use nullish coalescing
      totalPaid: totalPaid ?? 0,                // Use nullish coalescing
      overdueCount,
      averageInvoiceValue: averageInvoiceValue ?? 0,  // Use nullish coalescing
    };
  }, [formattedInvoices]);
  ...
  <Grid item xs={12} md={2.4} key={label}>
    <Paper sx={{ p: 3, textAlign: 'center', borderRadius: 3, bgcolor: theme.palette.background.paper }}>
      ...
      <Typography variant="h4" color={color}>{value !== undefined ? value: '0.00'}</Typography>
    </Paper>
  </Grid>
))}
