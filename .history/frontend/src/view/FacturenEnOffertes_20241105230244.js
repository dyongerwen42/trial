...
const InvoiceDashboard = () => {
  ...
  return (
    <Box sx={{ p: 6, maxWidth: '95vw', margin: '0 auto', bgcolor: theme.palette.background.default }}>
      {/* Export and Date Picker Section */}
      <Grid container spacing={4} alignItems="center" sx={{ mb: 6 }}>
        <Grid item xs={12} md={4} sx={{ display: 'flex', gap: 2 }}>
          <Button variant="contained" color="primary" startIcon={<ExcelIcon />} onClick={handleExportExcel}>
            Export Excel
          </Button>
      ...
      <Grid container spacing={4} sx={{ mb: 6 }}>
        {[
          ...
          <Grid item xs={12} sm={6} md={2.4} key={label}>
            <Paper sx={{ p: 4, textAlign: 'center', borderRadius: 3, bgcolor: theme.palette.background.paper, boxShadow: theme.shadows[10], height: 150 }}>
      ...
      <Grid container spacing={4} sx={{ mb: 6 }}>
        <Grid item xs={12} md={6}>
          <Paper sx={{ p: 4, boxShadow: theme.shadows[10], borderRadius: 3, height: 350 }}>
            <Typography variant="h6" sx={{ mb: 2, fontWeight: 'bold', textAlign: 'center' }}>Balans Trend</Typography>
            <Line data={lineChartData} options={{ responsive: true, maintainAspectRatio: false }} />
          </Paper>
        </Grid>
        <Grid item xs={12} md={3}>
          <Paper sx={{ p: 4, boxShadow: theme.shadows[10], borderRadius: 3, height: 350, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      ...
      <TableContainer component={Paper} sx={{ borderRadius: 4, boxShadow: theme.shadows[4], mb: 6 }}>
        <Table>
          <TableHead sx={{ bgcolor: theme.palette.primary.main }}>
            <TableRow>
              {['Naam', 'Bedrag', 'Status', 'Vervaldatum', 'Acties'].map((header) => (
                <TableCell key={header} sx={{ color: 'white', fontWeight: 'bold', padding: '20px', textAlign: 'center' }}>
          ...
          <TableBody>
            {filteredInvoices.slice(page * rowsPerPage, page * rowsPerPage + (rowsPerPage === 'all' ? filteredInvoices.length : rowsPerPage)).map((invoice, index) => (
              <TableRow key={index} sx={{ '&:hover': { bgcolor: theme.palette.action.hover, transition: 'background-color 0.3s' } }}>
                <TableCell sx={{ borderBottom: '1px solid #e0e0e0', padding: '20px', textAlign: 'center' }}>{invoice.name}</TableCell>
                <TableCell sx={{ borderBottom: '1px solid #e0e0e0', padding: '20px', textAlign: 'center' }}>€{(invoice.price || 0).toFixed(2)}</TableCell>
                <TableCell sx={{ borderBottom: '1px solid #e0e0e0', padding: '20px', textAlign: 'center', color: invoice.paymentStatus === 'betaald' ? theme.palette.success.main : theme.palette.error.main }}>
                  {invoice.paymentStatus}
                </TableCell>
                <TableCell sx={{ borderBottom: '1px solid #e0e0e0', padding: '20px', textAlign: 'center' }}>{invoice.dueDate ? invoice.dueDate.toLocaleDateString('nl-NL') : 'N/A'}</TableCell>
                <TableCell sx={{ borderBottom: '1px solid #e0e0e0', padding: '20px', textAlign: 'center' }}>
   ...
   return (
    <Box sx={{ p: 6, maxWidth: '95vw', margin: '0 auto', bgcolor: theme.palette.background.default }}>
      ...
      <Grid container spacing={4} sx={{ mb: 6 }}>
        <Grid item xs={12} md={6}>
          <Paper sx={{ p: 4, boxShadow: theme.shadows[10], borderRadius: 3, height: 400 }}>
            <Typography variant="h6" sx={{ mb: 3, fontWeight: 'bold', textAlign: 'center' }}>Balans Trend</Typography>
            <Line data={lineChartData} options={{ responsive: true, maintainAspectRatio: false }} />
          </Paper>
        </Grid>
        <Grid item xs={12} md={3}>
          <Paper sx={{ p: 4, boxShadow: theme.shadows[10], borderRadius: 3, height: 400, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <Typography variant="h6" sx={{ mb: 3, fontWeight: 'bold', textAlign: 'center' }}>Betaalstatus</Typography>
            <Pie data={pieChartData} options={{ responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom' } } }} />
          </Paper>
        </Grid>
        <Grid item xs={12} md={3}>
          <Paper sx={{ p: 4, boxShadow: theme.shadows[10], borderRadius: 3, height: 400 }}>
            <Typography variant="h6" sx={{ mb: 3, fontWeight: 'bold', textAlign: 'center' }}>Niet Betaalde Facturen per Maand</Typography>
            <Bar data={barChartData} options={{ responsive: true, maintainAspectRatio: false }} />
          </Paper>
        </Grid>
      </Grid>
      ...
      <TableContainer component={Paper} sx={{ borderRadius: 4, boxShadow: theme.shadows[4], mb: 6 }}>
        <Table>
          <TableHead sx={{ bgcolor: theme.palette.primary.main }}>
            <TableRow>
              {['Naam', 'Bedrag', 'Status', 'Vervaldatum', 'Acties'].map((header) => (
                <TableCell key={header} sx={{ color: 'white', fontWeight: 'bold', padding: '18px', textAlign: 'center' }}>
                  <Typography variant="h6">{header}</Typography>
                </TableCell>
              ))}
            </TableRow>
          ...
          <TableBody>
            {filteredInvoices.slice(page * rowsPerPage, page * rowsPerPage + (rowsPerPage === 'all' ? filteredInvoices.length : rowsPerPage)).map((invoice, index) => (
              <TableRow key={index} sx={{ '&:hover': { bgcolor: theme.palette.action.hover, transition: 'background-color 0.3s' } }}>
                <TableCell sx={{ borderBottom: '1px solid #e0e0e0', padding: '18px', textAlign: 'center' }}>{invoice.name}</TableCell>
                <TableCell sx={{ borderBottom: '1px solid #e0e0e0', padding: '18px', textAlign: 'center' }}>€{(invoice.price || 0).toFixed(2)}</TableCell>
                <TableCell sx={{ borderBottom: '1px solid #e0e0e0', padding: '18px', textAlign: 'center', color: invoice.paymentStatus === 'betaald' ? theme.palette.success.main : theme.palette.error.main }}>
                  {invoice.paymentStatus}
                </TableCell>
                <TableCell sx={{ borderBottom: '1px solid #e0e0e0', padding: '18px', textAlign: 'center' }}>{invoice.dueDate ? invoice.dueDate.toLocaleDateString('nl-NL') : 'N/A'}</TableCell>
                <TableCell sx={{ borderBottom: '1px solid #e0e0e0', padding: '18px', textAlign: 'center' }}>
                  <Tooltip title="Download">
                    <IconButton href={`http://localhost:5000/${invoice.file}`} target="_blank" color="primary">
                      <DescriptionIcon />
                    </IconButton>
