...
const InvoiceDashboard = () => {
  ...
  // Load the data (replace this with actual data or context if needed)
  const invoicesData = fakeData;

  // Memoize formatted data based on invoicesData
  const formattedInvoices = useMemo(() => {
    return invoicesData.flatMap((element) =>
  ...
  // Filter invoices based on filter and search term
  ...
  // Calculate KPI values based on filteredInvoices
  const kpis = useMemo(() => {
    const totalInvoices = filteredInvoices.length;
    const totalOutstanding = filteredInvoices.reduce(
      (sum, inv) => (inv.paymentStatus === 'niet betaald' ? sum + (inv.price || 0) : sum),
      0
    );
    const totalPaid = filteredInvoices.reduce(
      (sum, inv) => (inv.paymentStatus === 'betaald' ? sum + (inv.price || 0) : sum),
      0
    );
    const overdueCount = filteredInvoices.filter(
      (inv) => inv.paymentStatus === 'niet betaald' && new Date(inv.dueDate) < new Date()
    ).length;
  ...
  const handleChangeRowsPerPage = (event) => {
    setRowsPerPage(event.target.value === 'all' ? filteredInvoices.length : parseInt(event.target.value, 10));
    setPage(0);
  };
