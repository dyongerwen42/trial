...
const InvoiceDashboard = () => {
  ...
  const filteredInvoices = useMemo(() => {
    return formattedInvoices
      .filter((invoice) => {
        ...
        const matchesDate =
          (!dateRange[0] || invoice.dueDate >= dateRange[0]) &&
  ...
  }, [formattedInvoices, filter, searchTerm, dateRange, order, orderBy]);
  ...
  const kpis = useMemo(() => {
    ...
    const totalOutstanding = filteredInvoices.reduce(
      (sum, inv) => (inv.paymentStatus === 'niet betaald' ? sum + inv.price : sum),
      0
    );
    const totalPaid = filteredInvoices.reduce(
      (sum, inv) => (inv.paymentStatus === 'betaald' ? sum + inv.price : sum),
      0
    );
    const overdueCount = filteredInvoices.filter(
      (inv) => inv.paymentStatus === 'niet betaald' && inv.dueDate && inv.dueDate < new Date()
    ).length;
  ...
  const lineChartData = useMemo(
    () => ({
      labels: filteredInvoices.map((invoice) =>
        invoice.dueDate ? invoice.dueDate.toLocaleDateString('nl-NL') : ''
      ),
      datasets: [
        {
      ...
      datasets: [
        {
          ...
          data: Array.from({ length: 12 }, (_, i) =>
            filteredInvoices.reduce(
              (sum, inv) =>
                inv.paymentStatus === 'niet betaald' && inv.dueDate && inv.dueDate.getMonth() === i
                  ? sum + inv.price
                  : sum,
              0
            )
