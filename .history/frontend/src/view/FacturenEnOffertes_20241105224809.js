    ...
    <Box sx={{ p: 4, maxWidth: '95vw', margin: '0 auto', bgcolor: theme.palette.background.default }}>
      ...
      <Grid container spacing={3} sx={{ mb: 4 }}>
        <Grid item xs={12} md={6}>
          <Paper sx={{ p: 3, boxShadow: theme.shadows[10], borderRadius: 3, height: 350 }}>
        ...
        <Grid item xs={12} md={3}>
          <Paper sx={{ p: 3, boxShadow: theme.shadows[10], borderRadius: 3, height: 350, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <TableHead sx={{ bgcolor: theme.palette.primary.main }}>
            <TableRow>
              {['Naam', 'Bedrag', 'Status', 'Vervaldatum', 'Acties'].map((header) => (
                <TableCell key={header} sx={{ color: 'white', fontWeight: 'bold', py: 2, textAlign: 'center' }}>
          ...
          <TableBody>
            {filteredInvoices.slice(page * rowsPerPage, page * rowsPerPage + (rowsPerPage === 'all' ? filteredInvoices.length : rowsPerPage)).map((invoice, index) => (
              <TableRow key={index} sx={{ '&:hover': { bgcolor: theme.palette.action.hover, transition: 'background-color 0.3s' } }}>
                <TableCell sx={{ borderBottom: '1px solid #e0e0e0', padding: '16px', textAlign: 'center' }}>{invoice.name}</TableCell>
                <TableCell sx={{ borderBottom: '1px solid #e0e0e0', padding: '16px', textAlign: 'center' }}>€{(invoice.price || 0).toFixed(2)}</TableCell>
                <TableCell sx={{ borderBottom: '1px solid #e0e0e0', padding: '16px', textAlign: 'center', color: invoice.paymentStatus === 'betaald' ? theme.palette.success.main : theme.palette.error.main }}>
                  {invoice.paymentStatus}
                </TableCell>
                <TableCell sx={{ borderBottom: '1px solid #e0e0e0', padding: '16px', textAlign: 'center' }}>{invoice.dueDate ? invoice.dueDate.toLocaleDateString('nl-NL') : 'N/A'}</TableCell>
                <TableCell sx={{ borderBottom: '1px solid #e0e0e0', padding: '16px', textAlign: 'center' }}>
