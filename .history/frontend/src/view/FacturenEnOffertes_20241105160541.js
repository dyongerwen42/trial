      ...
      <Grid container spacing={4} sx={{ mb: 4 }}>
        {[
          ...
          <Grid item xs={12} sm={6} md={2.4} key={label}>
            <Paper sx={{ p: 3, textAlign: 'center', borderRadius: 3, bgcolor: theme.palette.background.paper, boxShadow: theme.shadows[4] }}>
              <Typography variant="h6" sx={{ fontWeight: 'bold', mb: 1 }}>{label}</Typography>
              <Typography variant="h4" sx={{ color }}>{typeof value === 'number' ? value.toFixed(2) : '0.00'}</Typography>
            </Paper>
          </Grid>
        ))}
      </Grid>
