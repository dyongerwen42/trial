        ...
        <Grid item xs={12} md={8}>
          <DateRangePicker
            ...
            renderInput={(startProps, endProps) => (
              <>
                <TextField {...startProps} sx={{ mr: 2, width: '48%' }} />
      ...
      <Grid container spacing={4} sx={{ mb: 4 }}>
        {[
          ...
          <Grid item xs={12} sm={6} md={2.4} key={label}>
            <Paper sx={{ p: 3, textAlign: 'center', borderRadius: 3, bgcolor: theme.palette.background.paper, boxShadow: theme.shadows[10], height: 120 }}>
      ...
      <Grid container spacing={4} sx={{ mb: 4 }}>
        <Grid item xs={12} md={6}>
          <Paper sx={{ p: 3, boxShadow: theme.shadows[10], borderRadius: 3, height: 350 }}>
            <Typography variant="h6" sx={{ mb: 2, fontWeight: 'bold', textAlign: 'center', color: theme.palette.warning.main }}>Balans Trend</Typography>
            <Line data={lineChartData} options={{ responsive: true, maintainAspectRatio: false }} />
          </Paper>
        </Grid>
        <Grid item xs={12} md={3}>
          <Paper sx={{ p: 3, boxShadow: theme.shadows[10], borderRadius: 3, height: 350, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <Typography variant="h6" sx={{ mb: 2, fontWeight: 'bold', color: theme.palette.warning.main }}>Betaalstatus</Typography>
            <Pie data={pieChartData} options={{ responsive: true, maintainAspectRatio: true, plugins: { legend: { position: 'bottom' } } }} />
          </Paper>
        </Grid>
        <Grid item xs={12} md={3}>
          <Paper sx={{ p: 3, boxShadow: theme.shadows[10], borderRadius: 3, height: 350 }}>
            <Typography variant="h6" sx={{ mb: 2, fontWeight: 'bold', textAlign: 'center', color: theme.palette.warning.main }}>Niet Betaalde Facturen per Maand</Typography>
          ...
          <TableHead sx={{ bgcolor: theme.palette.primary.main }}>
            <TableRow>
              {['Naam', 'Bedrag', 'Status', 'Vervaldatum', 'Acties'].map((header) => (
                <TableCell key={header} sx={{ color: 'white', fontWeight: 'bold', py: 2, px: 3 }}>
          ...
          <TableBody>
            {filteredInvoices.slice(page * rowsPerPage, page * rowsPerPage + (rowsPerPage === 'all' ? filteredInvoices.length : rowsPerPage)).map((invoice, index) => (
              <TableRow key={index} sx={{ '&:hover': { bgcolor: theme.palette.action.hover, transition: 'background-color 0.3s' } }}>
                ...
                <TableCell sx={{ borderBottom: '1px solid #e0e0e0', padding: '16px' }}>{invoice.dueDate ? invoice.dueDate.toLocaleDateString('nl-NL') : 'N/A'}</TableCell>
