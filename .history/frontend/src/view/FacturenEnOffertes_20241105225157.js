    ...
    <Box sx={{ p: 4, maxWidth: '95vw', margin: '0 auto', bgcolor: theme.palette.background.default }}>
        ...
        <Grid item xs={12} md={8}>
          <DateRangePicker
            ...
            renderInput={(startProps, endProps) => (
              <>
                <TextField {...startProps} sx={{ mr: 2, width: '45%' }} />
                <TextField {...endProps} sx={{ width: '45%' }} />
              </>
            )}
          />
        </Grid>
      ...
      <Grid container spacing={4} sx={{ mb: 4 }}>
        ...
        <Grid item xs={12} md={3}>
          <Paper sx={{ p: 3, boxShadow: theme.shadows[10], borderRadius: 3, height: 400, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
          ...
          <TableHead sx={{ bgcolor: theme.palette.primary.main }}>
            <TableRow>
              {['Naam', 'Bedrag', 'Status', 'Vervaldatum', 'Acties'].map((header) => (
                <TableCell key={header} sx={{ padding: '16px', fontWeight: 'bold', color: 'white' }}>
                  {header}
                </TableCell>
              ))}
            </TableRow>
