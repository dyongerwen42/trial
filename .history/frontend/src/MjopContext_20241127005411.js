...
const mjopReducer = (state, action) => {
  ...
  switch (action.type) {
    ...
    case EDIT_TASK: {
      ...
      const { elementId, task } = action.payload;
      if (!elementId) {
        console.warn("EDIT_TASK action missing elementId:", action.payload);
        return state;
      }
      return {
        ...state,
    ...
    case DELETE_TASK: {
      ...
      const { taskId, elementId: delElementId } = action.payload;
      if (!delElementId) {
        console.warn("DELETE_TASK action missing elementId:", action.payload);
        return state;
      }
      return {
        ...state,
        globalElements: state.globalElements.map((element) => {
          if (element.id === delElementId) {
            return {
              ...element,
              tasks: Array.isArray(element.tasks)
                ? element.tasks.filter((task) => task.id !== taskId)
                : [],
            };
          }
          return element;
...
export const MjopProvider = ({ children }) => {
  ...
  const editOfferGroupFunc = useCallback(
    (offerGroup) => {
  ...
  const deleteOfferGroupFunc = useCallback(
    (offerGroupId) => {
  ...
  const handleItemChange = useCallback(
    (taskId, elementId, field, value) => {
      logAction("Changing Item", { taskId, field, value, elementId });

      if (!elementId) {
        console.warn("handleItemChange called without a valid elementId.");
        setSnackbarMessage("Invalid element ID for task update.");
      ...
      dispatch({
        type: EDIT_TASK,
        payload: { elementId, task: { id: taskId, [field]: value } },
      });
    },
    [dispatch, logAction]
  );
  ...
  const handleDeleteItem = useCallback(
    (taskId, elementId) => {
      logAction("Deleting Item", { taskId, elementId });

      if (!elementId) {
        console.warn("handleDeleteItem called without a valid elementId.");
        setSnackbarMessage("Invalid element ID for task deletion.");
        setSnackbarOpen(true);
        return;
      }
  ...
  const contextValue = useMemo(
    () => ({
      ...
      editOfferGroup: editOfferGroupFunc,     // Corrected
      deleteOfferGroup: deleteOfferGroupFunc,   // Corrected
    }),
    [
      state,
      saveData,
