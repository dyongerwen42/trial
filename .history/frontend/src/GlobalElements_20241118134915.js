...
const extractGebreken = (
  ...
  // In Dataset 2, gebreken are directly grouped by severity
  return gebrekenFromState || {
    ernstig: [],
    serieus: [],
    gering: [],
  };
...
const GlobalElements = () => {
  ...
  const handleSelectElement = (element, isEditMode = false, fromTable = false) => {
    ...
    // Extract gebreken directly grouped by severity
    const availableGebreken = extractGebreken(element.name, inspectionTasks, element.gebreken);
  ...
  const handleAddCustomGebrek = () => {
    ...
    addGebrek(severity, gebrekName); // Directly add under severity
  ...
  const renderGebreken = () => {
    const availableGebreken = extractGebreken(newElement.name, inspectionTasks, newElement.gebreken);
            ...
            {availableGebreken[severity].length > 0 ? (
              <List>
                {availableGebreken[severity].map((gebrek, gebrekIndex) => {
                  const isChecked = newElement.gebreken[severity]?.includes(gebrek);

                  console.log(`Rendering checkbox for gebrek: ${gebrek} | Checked: ${isChecked}`);

                  return (
                    <ListItem key={`gebrek-${severity}-${gebrek}-${gebrekIndex}`}>
                      <FormControlLabel
                        control={
                          <Checkbox
                            checked={isChecked}
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} md={6}>
              <TextField
                ...
                onChange={(e) => setNewElement({ ...newElement, interval: e.target.value })}
                sx={{ mb: 2 }}
              />
            </Grid>
            ...
            <Grid item xs={12} md={6}>
              <FormControl fullWidth variant="outlined" sx={{ mb: 2 }}>
                ...
                <Select
                  ...
                  onChange={(e) => setNewElement({ ...newElement, categories: e.target.value })}
                  renderValue={(selected) => selected.join(", ")}
                  label="Selecteer Categorieën"
                >
                  {categories.map((category, categoryIndex) => (
                    <MenuItem key={`category-${category}-${categoryIndex}`} value={category}>
                      <Checkbox checked={newElement.categories?.includes(category)} />
            ...
            <Grid item xs={12} md={4}>
              <TextField
                ...
                onChange={(e) => setNewElement({ ...newElement, levensduur: e.target.value })}
                sx={{ mb: 2 }}
              />
            </Grid>
            ...
            <Grid item xs={12} md={4}>
              <TextField
                ...
                onChange={(e) => setNewElement({ ...newElement, aanschafDatum: e.target.value })}
                sx={{ mb: 2 }}
              />
            </Grid>
            ...
            <Grid item xs={12} md={4}>
              <TextField
                ...
                onChange={(e) => setNewElement({ ...newElement, vervangingsKosten: e.target.value })}
                sx={{ mb: 2 }}
              />
            </Grid>
          </Grid>
        ...
        return (
          <Grid container spacing={2}>
            ...
            {newElement.inspectionReport && (
              <Grid item xs={12}>
                <Typography variant="h6">Inspectierapport</Typography>
                <Typography variant="body1">
                        ...
                        >
                          {Object.keys(element.gebreken || {}).map(
                            (severity, severityIndex) => (
                              <ListItem
                                button
                                key={`severity-${element.id}-${severity}-${severityIndex}`}
                                onClick={() => {
                                  // Select all gebreken under this severity
                                  // Alternatively, you could have specific gebreken to select
                                  // For simplicity, let's assume clicking severity selects all gebreken under it
                                  // This might not be necessary in Dataset 2
                                  // So, perhaps do nothing or handle accordingly
                                  // For now, we'll select the element for editing
                                  handleSelectElement(element, true, false);
                                }}
                                sx={{
                                  padding: "12px 20px",
