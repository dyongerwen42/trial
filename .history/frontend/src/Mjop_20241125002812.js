...
const FullTaskManager = () => {
  ...
  const [selectedElements, setSelectedElements] = useState([]); // Added state for selected elements
  ...
  // Categorizing tasks by category
  ...
  const filteredCategorizedTasks = useMemo(() => {
    ...
    return Object.entries(categorizedTasks).reduce(
  ...
  const selectAllTasks = (category) => {
    const tasksInCategory = categorizedTasks[category].flatMap(
    ...
    >
      ...
      <Grid item xs={4}>
        ...
        >
          ...
          {/* Elements and Tasks */}
          {Object.entries(categorizedTasks).map(([category, elements]) => (
      ...
      <Grid item xs={8}>
        ...
        {/* Groups Overview */}
        <Paper elevation={4} sx={{ mt: 3, p: 3, borderRadius: 6 }}>
          <Typography variant="h5" gutterBottom>
            Groups
          </Typography>
          {offerGroups.length > 0 ? (
            offerGroups.map((group) => (
              <Paper key={group.id} elevation={3} sx={{ p: 2, mb: 2, borderRadius: 4 }}>
