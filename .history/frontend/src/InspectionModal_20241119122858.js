...
const InspectionModal = ({
  ...
  const handleAddFiles = async (newFiles, type) => {
    ...
    try {
      ...
      const updatedFiles = [...inspectionData[type], ...data.filePaths];
    ...
    try {
      ...
      const updatedMistakes = inspectionData.mistakes.map((mistake) =>
        mistake.id === mistakeId
          ? { ...mistake, images: [...mistake.images, ...data.filePaths] }
          : mistake
      );
      setInspectionData((prevState) => ({
        ...prevState,
  ...
  const handleTaskImageChange = async (taskId, newImages) => {
    ...
    try {
      ...
      const updatedTasks = tasks.map((task) =>
        task.id === taskId
          ? { ...task, images: [...task.images, ...data.filePaths] }
          : task
      );
      setTasks(updatedTasks);
    } catch (error) {
  ...
  const handleTaskDocumentChange = async (taskId, newDocuments) => {
    ...
    try {
      ...
      const updatedTasks = tasks.map((task) =>
        task.id === taskId
          ? { ...task, documents: [...task.documents, ...data.filePaths] }
          : task
      );
      setTasks(updatedTasks);
    } catch (error) {
  ...
  const calculateConditionScore = (mistakes) => {
    ...
    validMistakes.forEach((mistake) => {
      const { severity, intensityScore, omvang, replacementValue } = mistake;
      const severityKey = severity.toLowerCase();
      const intensity = parseInt(intensityScore, 10);
      ...
      const key = `${severityKey}-${intensity}`;
