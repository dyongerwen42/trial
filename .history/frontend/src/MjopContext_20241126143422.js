...
export const MjopProvider = ({ children }) => {
        ...
        const payload = {
          ...
          globalElements: Array.isArray(data.globalElements)
            ? data.globalElements.map((element) => ({
                ...element,
                gebreken: {
                  ernstig: Array.isArray(element.gebreken?.ernstig)
                    ? element.gebreken.ernstig
                    : [],
                  serieus: Array.isArray(element.gebreken?.serieus)
                    ? element.gebreken.serieus
                    : [],
                  gering: Array.isArray(element.gebreken?.gering)
                    ? element.gebreken.gering
                    : [],
                },
                inspectionReport: Array.isArray(element.inspectionReport)
                  ? element.inspectionReport.map((report) => ({
  ...
  const contextValue = useMemo(
    () => ({
      ...
      handleAddOfferGroup,
      handleEditOfferGroup,
      handleDeleteOfferGroup,
    }),
    [
      state,
      saveData,
