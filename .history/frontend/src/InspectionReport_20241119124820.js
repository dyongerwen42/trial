...
const InspectionReport = ({
  ...
  const handleInspectionChange = useCallback((elementId, reportId, field, value) => {
    setGlobalElements((prevElements) => {
      const updatedElements = prevElements.map((element) => {
        if (element.id === elementId) {
          setLastModifiedReportId(reportId);
          const updatedInspectionReport = Array.isArray(element.inspectionReport) ? element.inspectionReport.map((report) =>
            report.id === reportId
              ? { ...report, [field]: value }
              : report
          ) : [];
      ...
      // Zorg ervoor dat inspectionReport gesorteerd blijft
      updatedElements.forEach((element) => {
        if (Array.isArray(element.inspectionReport)) {
  ...
  const handleInspectionDoneChange = useCallback((elementId, reportId, value) => {
    setGlobalElements((prevElements) =>
      prevElements.map((element) => {
        if (element.id === elementId) {
          let updatedReports = Array.isArray(element.inspectionReport) ? element.inspectionReport.map((report) => {
          ...
          if (value) {
            const currentReport = Array.isArray(element.inspectionReport) ? element.inspectionReport.find(report => report.id === reportId) : null;
            const intervalInMonths = element.interval || 12;
            const nextInspectionDate = currentReport && currentReport.inspectionDate ? new Date(currentReport.inspectionDate) : new Date();

            if (currentReport && currentReport.inspectionDate) {
              nextInspectionDate.setMonth(nextInspectionDate.getMonth() + intervalInMonths);
            }

            const hasFutureInspection = Array.isArray(updatedReports) && updatedReports.some(report => {
              const inspectionDate = new Date(report.inspectionDate);
              return inspectionDate > nextInspectionDate;
            });

            if (!hasFutureInspection && currentReport) {
              const formattedDate = nextInspectionDate.toISOString().split('T')[0];
              updatedReports.push({
                id: uuidv4(),
                description: '',
                inspectionDone: false,
          ...
          return {
            ...element,
            inspectionReport: Array.isArray(updatedReports) ? updatedReports : [],
          };
        }
        return element;
      })
  ...
  return (
    <Box display="flex" height="70vh" overflow="auto" flexDirection="column" width="100%" p={2} bgcolor="#f9fafb">
      ...
      <InspectionModal
        ...
        // setGlobalElements={setGlobalElements} // Verwijder deze props
        // globalElements={globalElements}       // Verwijder deze props
        t={t}
      />
    </Box>
  );
