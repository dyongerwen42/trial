...
const GlobalElements = ({ t }) => {
  ...
  // Definieer modal functies eerst
  // Open photos modal
  ...
  const handleMaterialChange = (e) => {
    ...
    setNewElement((prev) => {
      ...
      if (
        ...
        updatedGebreken[prev.type][prev.material]
      ) {
        // Iterate over all severity categories and remove gebreken for the old material
        Object.keys(updatedGebreken[prev.type][prev.material]).forEach(
          (severity) => {
            if (updatedGebreken[prev.type][prev.material][severity]) {
              delete updatedGebreken[prev.type][prev.material][severity];
            }
          }
        );
        ...
        if (
          Object.keys(updatedGebreken[prev.type][prev.material]).length ===
          0
        ) {
          delete updatedGebreken[prev.type][prev.material];
        }

        // Remove type if no materials remain
        if (Object.keys(updatedGebreken[prev.type]).length === 0) {
          delete updatedGebreken[prev.type];
        }
      }
  ...
  // Define table columns with dependencies on modal functions
  ...
  // Define table columns with dependencies on modal functions
  // (Zorg ervoor dat handleOpenDocumentsModal en handleOpenPhotosModal gedefinieerd zijn)
  // Deze zijn al eerder gedefinieerd
