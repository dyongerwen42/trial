...
const GlobalElements = ({ t }) => {
  ...
  const handleSaveElement = async () => { // Maak deze functie asynchroon
    ...
    try {
      ...
      if (isEditing) {
        await handleEditElement(newElement); // Wacht op bewerken
        setSuccessMessage("Element succesvol bijgewerkt."); // Now works
      } else {
        await handleAddElement(newElement); // Wacht op toevoegen
        setSuccessMessage("Element succesvol toegevoegd."); // Now works
      }

      await saveData(); // Wacht op het opslaan van gegevens
