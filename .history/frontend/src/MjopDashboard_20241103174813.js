...
import GeneralInfo from "./GeneralInfo"; // Import GeneralInfo component
import { useMjopContext } from "./MjopContext"; // Import context hook
...
const MJOPDashboard = () => {
  ...
  const { state, dispatch } = useMjopContext(); // Use context to get state and dispatch
  const navigate = useNavigate();

  // Destructure generalInfo and error from the state
  ...
  return (
    <Box sx={{ mx: "auto", width: "100%", mt: 4, px: 2 }}>
      {/* Top Menu Buttons */}
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={4}>
          <Card sx={{ boxShadow: 4, p: 2, height: "100%" }}>
            <GeneralInfo generalInfo={generalInfo} /> {/* Pass generalInfo from context */}
          </Card>
        </Grid>
