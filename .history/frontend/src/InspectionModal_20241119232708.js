...
const InspectionModal = ({
    ...
    if (currentInspection) {
      ...
      if (element) {
        ...
        if (inspection) {
          ...
          console.warn(
            `Inspection with ID ${currentInspection.id} not found in element ${currentElementId}`
          );
        }
      } else {
  ...
  const handleAddFiles = async (newFiles, type) => {
    ...
    try {
      const { data } = await axios.post("http://34.34.100.96:5000/upload", formData, {
  ...
  const handleAddMistake = (gebrekName, severity) => {
    ...
    // Voeg het gebrek toe aan de globale context indien gewenst
    // In dit geval doen we dat niet om te voorkomen dat nieuwe categorieën worden toegevoegd
  ...
  const handleMistakeChange = (index, field, value) => {
    ...
    // Geen toevoeging aan globale context voor nieuwe categorieën
  };
  ...
  const handleRemoveMistake = (index) => {
    ...
    // Verwijder het gebrek uit de globale context indien gewenst
        ...
        const updatedInspectionReport = inspectionExists
          ? element.inspectionReport.map((inspection) =>
              inspection.id === currentInspection.id
                ? { ...inspection, ...inspectionData, tasks: tasks } // Voeg tasks toe aan de inspectie
                : inspection
            )
          : [
              ...element.inspectionReport,
              { ...inspectionData, tasks: tasks, id: uuidv4() },
            ];

        return {
          ...element,
          inspectionReport: Array.isArray(updatedInspectionReport) ? updatedInspectionReport : [updatedInspectionReport],
          // Verwijder het toevoegen van taken aan het element zelf
        };
      }
      return element;
    });
  ...
  return (
    <Modal
      ...
      // key={currentInspection.id} // Verwijder de key prop hier
      sx={{
        display: "flex",
        alignItems: "center",
                        ...
                        <Box display="flex" justifyContent="center" mt={1}>
                          <IconButton
                            ...
                            onClick={() => {
                              ...
                              link.href = `http://34.34.100.96:5000/${src}`;
                  ...
                  <Box display="flex" flexWrap="wrap" mt={2}>
                    {inspectionData.documents.map((src, index) => {
                      ...
                      return (
                        ...
                        >
                          <a
                            href={`http://34.34.100.96:5000/${src}`}
                            download
                            style={{ margin: "0 8px" }}
                              ...
                              >
                                ...
                                <Box display="flex" justifyContent="center" mt={1}>
                                  <IconButton
                                    ...
                                    onClick={() => {
                                      const link = document.createElement("a");
                                      link.href = `http://34.34.100.96:5000/${image}`;
