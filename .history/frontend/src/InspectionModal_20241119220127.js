    ...
    if (currentInspection) {
      ...
      if (element) {
        ...
        if (inspection) {
          ...
          setTasks(Array.isArray(inspection.tasks) ? inspection.tasks : []); // Aangepast
        }
      }
    }
  ...
  const handleSave = () => {
    ...
    const updatedElements = globalElements.map((element) => {
      if (element.id === currentElementId) {
        ...
        let updatedInspectionReport;
        if (inspectionExists) {
          updatedInspectionReport = element.inspectionReport.map((inspection) =>
            inspection.id === currentInspection.id
              ? { ...inspection, ...inspectionData, tasks: Array.isArray(tasks) ? tasks : [] }
              : inspection
          );
        } else {
          updatedInspectionReport = [
            ...element.inspectionReport,
            { ...inspectionData, id: uuidv4(), tasks: Array.isArray(tasks) ? tasks : [] },
          ];
        }
                      ...
                      <Grid item xs={12} sm={6}>
                        <Autocomplete
                          ...
                          options={(() => {
                            ...
                            return Array.from(new Set(allGebreken));
                          })()}
                          ...
                          onInputChange={(event, newInputValue) => {
                            if (event && event.type === 'blur' && newInputValue) {
                              const currentElement = globalElements.find(
                                (el) => el.id === currentElementId
                              );
                              if (
                                currentElement &&
                                !currentElement.gebreken.ernstig.includes(newInputValue) &&
                                !currentElement.gebreken.serieus.includes(newInputValue) &&
                                !currentElement.gebreken.gering.includes(newInputValue)
                              ) {
                                // Voeg de nieuwe categorie toe aan de juiste ernst
                                addGebrekToElement(currentElementId, "ernstig", [newInputValue]);
                              }
                            }
                          }}
                          renderInput={(params) => (
