...
const Spaces = () => {
  ...
  const { globalSpaces, globalElements } = state; // Destructure globalSpaces and globalElements from state
                      ...
                      space.photos.map((photo, index) => (
                        <Tooltip title={`Bekijk Foto ${index + 1}`} key={`${space.id}-photo-${index}`}>
                          <IconButton
                            color="primary"
                            onClick={() =>
                              handleOpenFile(`http://34.34.100.96:5000/${photo}`)
                            }
                          >
                            <ImageIcon />
                          </IconButton>
                      ...
                      space.documents.map((document, index) => (
                        <Tooltip title={`Bekijk Document ${index + 1}`} key={`${space.id}-document-${index}`}>
                          <IconButton
                            color="primary"
                            onClick={() =>
                              handleOpenFile(`http://34.34.100.96:5000/${document}`)
                            }
                          >
                            <PictureAsPdfIcon />
                          </IconButton>
                    ...
                    {space.image && space.annotations?.length > 0 && (
                      <Tooltip title="Bekijk annotaties">
                        <IconButton
                          color="primary"
                          onClick={() =>
                            handleOpenAnnotation(`http://34.34.100.96:5000/${space.image}`, space.annotations)
                          }
                        >
                          <VisibilityIcon />
                        </IconButton>
