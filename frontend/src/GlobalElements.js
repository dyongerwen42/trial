...
const GlobalElements = ({ t }) => {
  ...
  const handleSelectElement = (
    ...
    if (selectedSpace && typeof selectedSpace.image === "string") {
      const imageWithAnnotations = selectedSpace.image.startsWith("http")
        ? selectedSpace.image
        : `http://34.34.100.96:5000/${selectedSpace.image.replace(/\\/g, "/")}`;
  ...
  const handleOpenDocumentsModal = (documents) => {
    setSelectedDocuments(
      documents.map((doc) =>
        typeof doc === "string"
          ? `http://34.34.100.96:5000/${doc.replace(/\\/g, "/")}`
  ...
  const handleOpenPhotosModal = (photos) => {
    setSelectedPhotos(
      photos.map((photo) =>
        typeof photo === "string"
          ? `http://34.34.100.96:5000/${photo.replace("\\", "/")}`
                ...
                <Select
                  ...
                  onChange={(e) => {
                    ...
                    if (selectedSpace && typeof selectedSpace.image === "string") {
                      const imageWithAnnotations = selectedSpace.image.startsWith("http")
                        ? selectedSpace.image
                        : `http://34.34.100.96:5000/${selectedSpace.image.replace(
                            /\\/g,
                            "/"
                          )}`;
                      setSelectedImage(imageWithAnnotations);
              ...
              <FormControl fullWidth>
                ...
                <Box display="flex" flexWrap="wrap" mt={2}>
                  {Array.isArray(newElement.photos) && newElement.photos.map((src, index) => {
                    const imgSrc =
                      typeof src === "string"
                        ? `http://34.34.100.96:5000/${src.replace("\\", "/")}`
